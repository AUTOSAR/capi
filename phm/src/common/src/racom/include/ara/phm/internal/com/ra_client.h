...
/// ----------------
/// @file       ra_client.h
/// @brief      used by SupervisionManager to send request.
/// @details
/// @date       2024-06-05
/// @author     wangyanlong
/// @version    1.2.0
...
/// @unit_description=used by SupervisionManager to send request.
...
#ifndef ARA_PHM_INTERNAL_RACOM_CLIENT_H_
#define ARA_PHM_INTERNAL_RACOM_CLIENT_H_
...
public:
    ...
    static std::unique_ptr< Client > GetInstanceUnique(
        std::function< void(RecoveryResult const result, ara::core::String const& instance) > const
...
private:
    ...
    /// @param recoveryResultHandler Function called when to process recovery result.
    /// @trace_id_sr=SR_PHM_01027
    /// @needwork = dda
    explicit Client(std::function< void(RecoveryResult const result, ara::core::String const& instance) > const&
                        recoveryResultHandler) noexcept
        : BaseClient{GetIpcPhmRaCmClient()}
        , recoveryResultHandler_{recoveryResultHandler}
        , notifyInfo_{}
        , messageSerializer_{} {};
...
private:
    /// @brief The use callback to process the recovery result.
    /// @trace_id_sr=SR_PHM_01027
    /// @needwork = dda
    std::function< void(RecoveryResult result, ara::core::String instance) > recoveryResultHandler_;

    ///@brief <identifier of RecoveryAction, notify info>
    /// @trace_id_sr=SR_PHM_01027
    /// @needwork = dda
...
#endif  // ARA_PHM_INTERNAL_RACOM_CLIENT_H_
