...
Chunk MessageSerializer::Serialize() const noexcept
{
    ...
    if (isoft::serialize::Serialize(buffer, msg_.eventType, msg_.identifier, msg_.timestamp) < 0) {
        std::ignore = buffer;
...
int32_t MessageSerializer::Deserialize(Chunk& chunk) noexcept
{
    ...
    if (isoft::serialize::Deserialize(buffer, msg_.eventType, msg_.identifier, msg_.timestamp) < 0) {
