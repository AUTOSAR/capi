...
/// File description:
/// ----------------
/// @file       approval_process.cpp
/// @brief      ApprovalProcess class implementation
/// @details
/// @date       2024-07-20
/// @author     hanzhibo
/// @version    1.2.0
...
/// @unit_description=ApprovalProcess class implementation
/// @endcode
///
/// ================================================================
...
/// @throws no
void ApprovalProcess::_signalFromVsm(bool const& safeToUpdate, pkgmgr::SafetyPolicyType const& vsmSafetyPolicy)
{
...
/// @throws no
void ApprovalProcess::_signalFromVdia(bool const& approval, pkgmgr::SafetyPolicyType const& safetyCondition)
{
...
/// @return bool
/// @throws no
bool ApprovalProcess::Run(pkgmgr::SafetyPolicyType const& safetyCondition)
{
...
/// @return bool
/// @throws no
bool ApprovalProcess::_vsmCondition(pkgmgr::SafetyPolicyType const& safetyCondition)
{
    vsmEvent_.Add(1);
    FsmManager* const fsm{FsmManager::GetInstance()};
    while (true) {
...
/// @return bool
/// @throws no
bool ApprovalProcess::_driverNotification()
{
    ...
    if (!driverApproval_) {
        LOG_INFO << "vehicle package config notification driver process= " << BoolToStr(driverApproval_).data();
        return true;
    }
    ...
    /// TODO(yunfei) Whether to reset to default callback after lifecycle ends
    vdia_->SetApprovalCB(actionCB);

    LOG_INFO << "notification: approval to driver";
    std::ignore = vdia_->ApprovalRequired.Update(true);
    // vdia_ => SafetyPolicy // Driver awareness not required for the step execution strategy; will be determined later by vehicle state management
