...
# @brief      Script to run clang-tidy on all C++ source files in a project
# @details
# @date       2024-10-11
...
while read -r file; do
    ...
    if [[ "$file" == *"/files/"* || "$file" == *"/build/"* || "$file" == *"/.build/"* || "$FILE_PATH" == "CMake"* || "$file" == *"/doc/"* || "$file" == *"/qac/"* || "$file" == *"/tests/"* || "$file" == *"/examples/"* ]]; then
    # if [[ "$file" == *"/files/"* || "$file" == *"/build/"* || "$FILE_NAME" == *"test"* || "$FILE_PATH" == *"examples"* || "$FILE_PATH" == "CMake"* ]]; then
