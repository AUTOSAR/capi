...
ActionItemStopSM::ActionItemStopSM(core::String smFQN) noexcept
    : ActionItemBase{ActionType::kStopStateMachine}, smFQN_{std::move(smFQN)}, processContext_{}
{
    GetLog().LogInfo() << "ActionItemStopSM::ActionItemStopSM(), smFQN_:" << smFQN_.c_str();
}
...
void ActionItemStopSM::StartExecute(uint32_t const processID) noexcept
{
    GetLog().LogInfo() << "ActionItemStopSM::StartExecute(), smFQN_:" << smFQN_.c_str();
    assert(stopStateMachineHandler_);
...
void ActionItemStopSM::OnSuccess() noexcept
{
    GetLog().LogInfo() << "ActionItemStopSM::OnSuccess(), smFQN_:" << smFQN_.c_str();
}

/// @brief Handling after action execution fails
void ActionItemStopSM::OnFailure() noexcept
{
    GetLog().LogInfo() << "ActionItemStopSM::OnFailure(), smFQN_:" << smFQN_.c_str();
}

/// @brief Callback when the Future corresponding to Action is ready
/// @param processID Processing sequence number
void ActionItemStopSM::OnActionFutureReady(uint32_t const processID) noexcept
{
    GetLog().LogInfo() << "ActionItemStopSM::OnActionFutureReady(), smFQN_:" << smFQN_.c_str();
    ...
    if (res) {  // Success
        GetLog().LogDebug() << "ActionItemStopSM::OnActionFutureReady(), succeed to execute:"
                            << ActionItemToString().c_str();
        OnSuccess();
        processContext_->groupNum--;
        GetLog().LogDebug() << "ActionItemStopSM::OnActionFutureReady(), groupNum is updated to:"
                            << processContext_->groupNum << "smFQN_:" << smFQN_.c_str();
    } else {
        processContext_->hasErrorResult = true;
        GetLog().LogError() << "ActionItemStopSM::OnActionFutureReady(), failed with error:"
    ...
    if (processContext_->hasErrorResult) {          // Whether there is an error
        ...
        GetLog().LogDebug()
            << "ActionItemStopSM::OnActionFutureReady(), groupNum is 0, so try to ContinueProcessingActionList "
...
void ActionItemStopSM::_setupExcuteFuture(uint32_t const processID)
{
    _SetExcuteFuture(stopStateMachineHandler_(smFQN_));
    std::ignore = _GetExcuteFuture().then([this, processID]() {
        ...
        if (processID != processContext_->processID) {  // Ignore callbacks not for the current processing list
            GetLog().LogDebug()
                << "ActionItemStopSM::futureReadyContinuation(), return immediately because processID is" << processID
