...
namespace diag {
namespace dmd {
FileTransfer::FileTransfer(uint16_t const& instanceId, uint32_t const& serviceInstanceId)
    : agentPtr_{std::make_unique< isoft::dm::dic::FileTransferServiceAgent >(instanceId, serviceInstanceId)}
{
}
...
///  @return A future with either the access strategy to be used during reading and the file sizes to be
///          reported to the UDS client for a positive response message) or an UDS NRC value (for an negative
///        response message)
///  @return uncompressed size: If set to 0 - the parameter will not be reported to the diagnostic client
///  @return compressed size: If set to 0 - the parameter will not be reported to the diagnostic client. If no
...
isoft::uds::Result< FileTransfer::FileSizes > FileTransfer::RequestReadFile(
    std::string fileName,
    ...
    CancellationHandler cancellationHandler) noexcept
{
    ...
    if (agentPtr_.get() != nullptr) {
        ara::core::Result< isoft::dm::FileSizes > result = agentPtr_->RequestReadFile(
            ara::core::String(fileName), dataFormatIdentifier, comMetaInfo, cancellationEventPtr);
        if (result.HasValue()) {
            FileTransfer::FileSizes fileSize{};
            fileSize.uncompressedSize = result.Value().uncompressedSize;
            fileSize.compressedSize   = result.Value().compressedSize;
            fileSize.vecMD5 = std::vector< uint8_t >(result.Value().vecMD5.begin(), result.Value().vecMD5.end());
            return isoft::uds::Result< FileTransfer::FileSizes >::FromValue(fileSize);
        }
        if (result.Error().Value() < 0) {
            return isoft::uds::Result< FileTransfer::FileSizes >::FromError(isoft::serialize::kInt32_0x10);
        }
        return isoft::uds::Result< FileTransfer::FileSizes >::FromError(result.Error().Value());
    }
    return isoft::uds::Result< FileTransfer::FileSizes >::FromError(isoft::serialize::kInt32_0x10);
}
...
isoft::uds::Result< FileTransfer::RequestFileResponse > FileTransfer::RequestReadDirectory(
    std::string directoryName, MetaInfoMap const& metaInfo, CancellationHandler cancellationHandler) noexcept
{
    isoft::dm::MetaInfoMap comMetaInfo{};
    for (auto&& pair : metaInfo) {
        ara::core::String key{pair.first};
    ...
    if (agentPtr_.get() != nullptr) {
        ara::core::Result< isoft::dm::RequestFileResponse > result
            = agentPtr_->RequestReadDirectory(ara::core::String(directoryName), comMetaInfo, cancellationEventPtr);
        if (result.HasValue()) {
            FileTransfer::RequestFileResponse fileSize{};
            fileSize.fileSize = result.Value().fileSize;
            fileSize.vecMD5   = std::vector< uint8_t >(result.Value().vecMD5.begin(), result.Value().vecMD5.end());
            return isoft::uds::Result< FileTransfer::RequestFileResponse >::FromValue(fileSize);
        }
        if (result.Error().Value() < 0) {
            return isoft::uds::Result< FileTransfer::RequestFileResponse >::FromError(isoft::dm::kGeneralReject);
        }
        return isoft::uds::Result< FileTransfer::RequestFileResponse >::FromError(result.Error().Value());
    }
    return isoft::uds::Result< FileTransfer::RequestFileResponse >::FromError(isoft::dm::kGeneralReject);
}
...
isoft::uds::Result< std::vector< uint8_t > > FileTransfer::RequestWriteFile(
    std::string fileName,
    std::uint8_t dataFormatIdentifier,
    FileSizes fileSizes,
    WriteFileMode mode,
    ...
    CancellationHandler cancellationHandler) noexcept
{
    ...
    std::shared_ptr< isoft::dm::CancellationEvent > cancellationEventPtr{
        std::make_shared< isoft::dm::CancellationEvent >()};
    cancellationHandler.SetNotifier([&cancellationEventPtr]() { cancellationEventPtr->Notifier(); });
    isoft::dm::FileSizes comFileSizes{};
    comFileSizes.uncompressedSize = fileSizes.uncompressedSize;
    comFileSizes.compressedSize   = fileSizes.compressedSize;
    isoft::dm::dic::WriteFileMode comWriteFileMode{static_cast< isoft::dm::dic::WriteFileMode >(mode)};
    if (agentPtr_.get() != nullptr) {
        ara::core::Result< ara::core::Vector< uint8_t > > result
            = agentPtr_->RequestWriteFile(ara::core::String(fileName), dataFormatIdentifier, comFileSizes,
                                          comWriteFileMode, comMetaInfo, cancellationEventPtr);
        if (result.HasValue()) {
            return isoft::uds::Result< std::vector< uint8_t > >::FromValue(
                std::vector< uint8_t >(result.Value().begin(), result.Value().end()));
        }
        if (result.Error().Value() < 0) {
            return isoft::uds::Result< std::vector< uint8_t > >::FromError(isoft::dm::kGeneralReject);
        }
        return isoft::uds::Result< std::vector< uint8_t > >::FromError(result.Error().Value());
    }
    return isoft::uds::Result< std::vector< uint8_t > >::FromError(isoft::serialize::kInt32_0x10);
}
...
isoft::uds::Result< FileTransfer::RequestFileResponse > FileTransfer::RequestResumeWriteFile(
    std::string fileName,
    std::uint8_t dataFormatIdentifier,
    FileSizes fileSizes,
    MetaInfoMap const& metaInfo,
    CancellationHandler cancellationHandler) noexcept
{
    ...
    comFileSizes.compressedSize   = fileSizes.compressedSize;
    if (agentPtr_.get() != nullptr) {
        ara::core::Result< isoft::dm::RequestFileResponse > result = agentPtr_->RequestResumeWriteFile(
            ara::core::String(fileName), dataFormatIdentifier, comFileSizes, comMetaInfo, cancellationEventPtr);
        if (result.HasValue()) {
            FileTransfer::RequestFileResponse fileSize{};
            fileSize.fileSize = result.Value().fileSize;
            fileSize.vecMD5   = std::vector< uint8_t >(result.Value().vecMD5.begin(), result.Value().vecMD5.end());
    ...
    return isoft::uds::Result< FileTransfer::RequestFileResponse >::FromError(isoft::serialize::kInt32_0x10);
}
...
isoft::uds::Result< std::vector< uint8_t > > FileTransfer::DeleteFile(std::string fileName,
                                                                      MetaInfoMap const& metaInfo,
                                                                      CancellationHandler cancellationHandler) noexcept
{
    isoft::dm::MetaInfoMap comMetaInfo{};
    ...
    if (agentPtr_.get() != nullptr) {
        ara::core::Result< ara::core::Vector< uint8_t > > result
            = agentPtr_->DeleteFile(ara::core::String(fileName), comMetaInfo, cancellationEventPtr);
        if (result.HasValue()) {
            return isoft::uds::Result< std::vector< uint8_t > >::FromValue(
                std::vector< uint8_t >(result.Value().begin(), result.Value().end()));
        }
...
///  requested Span size.
/// @param[in] response_data  The view over a pre-allocated by AraDiag memory to write in this data
/// chunk.
...
/// positive
/// response message) or readied with ErrorCode from NrcErrc (for an negative response
...
isoft::uds::Result< std::vector< std::uint8_t > > FileTransfer::Read(std::uint8_t blockSequenceCounter,
                                                                     ...
                                                                     CancellationHandler cancellationHandler) noexcept
{
    ...
    cancellationHandler.SetNotifier([&cancellationEventPtr]() { cancellationEventPtr->Notifier(); });
    if (agentPtr_.get() != nullptr) {
        ara::core::Result< std::vector< std::uint8_t > > result
            = agentPtr_->Read(numBytesToReturn, comMetaInfo, cancellationEventPtr);
        if (result.HasValue()) {
            std::vector< std::uint8_t > tem;
            for (auto const& element : result.Value()) {
                tem.emplace_back(element);
            }

            return isoft::uds::Result< std::vector< std::uint8_t > >::FromValue(tem);
        }
        if (result.Error().Value() < 0) {
...
/// @param[in] transfer_request_parameter_record This parameter record contains parameter(s), which are
/// required by
...
///          for a positive response message) or readied with ErrorCode from NrcErrc (for an
...
isoft::uds::Result< std::vector< std::uint8_t > > FileTransfer::ExitRead(
    ...
    CancellationHandler cancellationHandler) noexcept
{
    ...
    std::shared_ptr< isoft::dm::CancellationEvent > cancellationEventPtr{
        std::make_shared< isoft::dm::CancellationEvent >()};
    cancellationHandler.SetNotifier([&cancellationEventPtr]() { cancellationEventPtr->Notifier(); });
    isoft::dm::dic::DataTransferExitType comExitType{static_cast< isoft::dm::dic::DataTransferExitType >(exitType)};
    if (agentPtr_.get() != nullptr) {
        ara::core::Result< std::vector< std::uint8_t > > result
            = agentPtr_->ExitRead(comExitType, transferRequestParameterRecord, comMetaInfo, cancellationEventPtr);
        if (result.HasValue()) {
...
/// @param[in] metaInfo Contains additional meta information
/// @param[in] cancellationHandler  Informs if the current conversation is canceled
/// @return A Future, which either is of type void or readied with ErrorCode from NrcErrc (for an
///          negative response message).
/// @error Any Application responded with a UDS NRC value from NrcErrc
/// @pre -
/// @reentrant FALSE
/// @traceid{SWS_DM_01546}@tracestatus{draft}
isoft::uds::Result< void > FileTransfer::Write(std::uint8_t blockSequenceCounter,
                                               ...
                                               CancellationHandler cancellationHandler) noexcept
{
    ...
    cancellationHandler.SetNotifier([&cancellationEventPtr]() { cancellationEventPtr->Notifier(); });
    if (agentPtr_.get() != nullptr) {
        ara::core::Result< void > result = agentPtr_->Write(requestData, comMetaInfo, cancellationEventPtr);
        if (result.HasValue()) {
            return {};
        }
        if (result.Error().Value() < 0) {
...
/// for a positive response message) or readied with ErrorCode from NrcErrc (for an negative
...
isoft::uds::Result< std::vector< std::uint8_t > > FileTransfer::ExitWrite(
    isoft::uds::server::DataTransferExitType exitType,
    std::vector< std::uint8_t > transferRequestParameterRecord,
    MetaInfoMap const& metaInfo,
    CancellationHandler cancellationHandler) noexcept
{
    ...
    if (agentPtr_.get() != nullptr) {
        ara::core::Result< std::vector< std::uint8_t > > result
            = agentPtr_->ExitWrite(comExitType, transferRequestParameterRecord, comMetaInfo, cancellationEventPtr);
        if (result.HasValue()) {
            return isoft::uds::Result< std::vector< std::uint8_t > >::FromValue(result.Value());
        }
        if (result.Error().Value() < 0) {
