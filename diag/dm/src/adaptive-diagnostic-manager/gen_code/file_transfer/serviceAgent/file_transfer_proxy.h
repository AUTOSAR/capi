...
#ifndef __SERVICEAGENT_FILETRANSFERPROXY_H_
#define __SERVICEAGENT_FILETRANSFERPROXY_H_
...
using DataTransferExitType       = std::uint32_t;
using WriteFileMode              = std::uint32_t;
constexpr WriteFileMode kAdd     = 0U;  ///< The file shall be added only if not existing
constexpr WriteFileMode kReplace = 1U;  ///< The file shall be added in any case, if already exists, will be replaced
class FileTransferServiceProxy : public BussinessClientProxy
{
public:
    explicit FileTransferServiceProxy(uint16_t const& instanceId, uint32_t const& serviceInstanceId);
...
#endif  // __SERVICEAGENT_FILETRANSFERPROXY_H_