...
while read -r file; do
    if [[ "$file" == *"/misc/"* || "$file" == *"/build/"* || "$file" == *"/.build*/"* || "$file" == *"fastdds_idl"* || "$file" == *"/idlgen/"* || "$file" == *"/icc/icc/"* || "$file" == *"/fsomeip/fsomeip/"* ]]; then
        echo "Formatting File ignored : $file"
    else
        # 打印文件的完整路径
