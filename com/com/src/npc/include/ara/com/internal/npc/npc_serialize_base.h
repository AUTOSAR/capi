...
#ifndef __COM_NPC_SERIALIZED_BASE_H
#define __COM_NPC_SERIALIZED_BASE_H
...
enum ErrorCode
{
    ...
    kInvalidPoint = -4,  ///< Invalid position
...
{
    kSessionHandlingActive,   ///< Active
    kSessionHandlingInactive  ///< Inactive
};
...
struct TransformationProps
{
    ...
    /// @brief Whether to serialize legacy strings -- to be deprecated
    ...
    /// @brief Get sequence/associative map container length field size (extension fixed-length array) @ref [TPS_MANI_03070]
    /// @details shall be either 0, 1, 2 or 4 @ref [constr_3353]
    /// @details 0 is only allowed to be used if a fixed size array @ref [constr_3447]
    /// @details in case of TLV, sizeof*LengthField shall have an identical value and greater than 0 @ref [constr_1628]
    /// @return uint8_t -- default 4 @ref [SWS_CM_00258] [SWS_CM_10258]
    ...
    /// @brief Get struct tag length field size @ref [TPS_MANI_03072]
    ...
    /// @brief Get struct tag selector field size @ref [TPS_MANI_03074]
...
/// @brief Template type -- check if scalar (basic) type
/// @tparam T Value type
template < typename T >
using IsScalar = std::is_scalar< T >;
/// @brief Template type -- check if string type (false)
/// @tparam T Value type
template < typename T >
struct IsString : std::false_type
{
};
/// @brief Template type -- check if string type (true)
template <>
struct IsString< ara::core::String > : std::true_type
{
};
/// @brief Template type -- check if struct (parameter list + enumerable members) type (false)
/// @tparam T Value type
...
/// @brief Template type -- check if struct (parameter list + enumerable members) type (true)
template < typename T >
...
/// @brief Template type -- check if struct (enumerable members) type (false)
/// @tparam T Value type
/// @tparam Tagged A tag to enable SFINAE.
template < typename T, typename Tagged = void >
...
/// @brief Template type -- check if struct (enumerable members) type (true)
template < typename T >
...
/// @brief Template type -- check if struct (with length field + enumerable members) type (true)
template < typename T >
...
/// @brief Template type -- check if struct (with tag field + length field + enumerable members) type (false)
/// @tparam T Value type
/// @tparam Tagged A tag to enable SFINAE.
template < typename T, typename Tagged = void >
...
/// @brief Template type -- check if struct (with tag field + length field + enumerable members) type (true)
template < typename T >
...
/// @brief Template type -- check if struct (S2S conversion) type (false)
/// @tparam T Value type
/// @tparam Tagged A tag to enable SFINAE.
template < typename T, typename Tagged = void >
...
/// @brief Template type -- check if struct (S2S conversion) type (true)
template < typename T >
...
/// @brief Template type -- check if container type (false)
/// @tparam T Value type
...
/// @brief Template type -- check if container type (true)
template < typename... T, template < typename... > class C >
...
using IsAssociativeContainer
    = std::conditional_t< IsContainer< T, ara::core::Map >::value || IsContainer< T, std::map >::value
                          ...
                          std::true_type,
                          std::false_type >;
/// @brief Template type -- check if fixed-length array length field can be extended (false)
/// @tparam T
template < typename T, typename = void >
struct EnableFixedArrayLengthField : std::false_type
{
};
/// @brief Template type -- check if fixed-length array length field can be extended (true)
/// @tparam T
template < typename T >
...
/// @brief Template type -- check if fixed-length string length field can be extended (false)
/// @tparam T
template < typename T, typename = void >
struct EnableFixedStringLengthField : std::false_type
{
};
/// @brief Template type -- check if fixed-length string length field can be extended (true)
/// @tparam T
template < typename T >
