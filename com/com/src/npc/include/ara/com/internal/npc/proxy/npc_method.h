...
#ifndef __COM_NPC_PROXY_METHOD_H
#define __COM_NPC_PROXY_METHOD_H
...
/// @tparam mode Method mode -- default bidirectional method
...
/// @brief Type alias -- bidirectional void return method
/// @tparam Owner Service element owner
/// @tparam Desc Description info
/// @tparam T Method signature type
template < typename Owner, typename Desc, typename T >
using NpcVMethod = NpcMethod<
    Owner,
    Desc,
    std::enable_if_t<
...
/// @brief Type alias -- bidirectional method
/// @tparam Owner Service element owner
/// @tparam Desc Description info
/// @tparam T Method signature type
template < typename Owner, typename Desc, typename T >
using NpcRMethod = NpcMethod<
    Owner,
    Desc,
    std::enable_if_t<
...
class NpcMethod< Owner, Desc, void(Args...), ara::com::internal::proxy::MethodMode::kNonReturn >
...
public:
    ...
    explicit NpcMethod(Owner& owner) noexcept : owner_{owner}
    {
    ...
    /// @brief Destructor
    ~NpcMethod() noexcept override { ComLogTrace("destroy bind proxy method", GenArg(Desc::methodId())); }
...
private:
    ...
    {
        TimeRecorder(TimeNM::Timer, "NPCRequest(F)");
        ...
        if (request == nullptr) {
            ComLogError("call bind method error: create request failed", GenArg(Desc::methodId()), GenArg(sessionId),
                        GenArg(args...));
            MonitorTrace(runtime::GetInstance(), trace::userdefine::kSerialize, trace::ServiceType::kProxy,
        ...
        MonitorTrace(runtime::GetInstance(), trace::userdefine::kSerialize, trace::ServiceType::kProxy,
                     Desc::serviceId(), owner_.instanceId_, Desc::serviceIdentifier, Desc::methodId(), "methodName",
                     false, sessionId, static_cast< uint8_t >(trace::MsgType::kMethodRequestNoReturn), 0U,
        ...
        if (ret < 0) {
            ...
            MonitorTrace(runtime::GetInstance(), trace::userdefine::kSendRequest, trace::ServiceType::kProxy,
                         Desc::serviceId(), owner_.instanceId_, Desc::serviceIdentifier, Desc::methodId(), "methodName",
                         false, sessionId, static_cast< uint8_t >(trace::MsgType::kMethodRequestNoReturn),
                         static_cast< uint32_t >(ComErrc::kCommunicationStackError), trace::FormatArgs(args...));
            return;
...
/// @brief Template type -- method implementation type -- specialization for bidirectional method
/// @code{.isoft}
/// export_level=/COM/SoC/SOA
/// @endcode
/// @tparam Owner Service element owner
...
class NpcMethod< Owner, Desc, R(Args...), ara::com::internal::proxy::MethodMode::kHasReturn >
...
private:
    ...
    {
        TimeRecorder(TimeNM::Timer, "NPCRequest(R)");
        ComLogTrace("call bind method", GenArg(Desc::methodId()), GenArg(args...));
        ...
        MonitorTrace(runtime::GetInstance(), trace::userdefine::kSerialize, trace::ServiceType::kProxy,
                     Desc::serviceId(), owner_.instanceId_, Desc::serviceIdentifier, Desc::methodId(), "methodName",
                     false, sessionId, static_cast< uint8_t >(trace::MsgType::kMethodRequest), 0U,
                     trace::FormatArgs(args...));
    ...
    static int32_t OnResponse(npc_app_t* a, void* ud, npc_message_t* m) noexcept
    {
        std::ignore = a;
        TimeRecorder(TimeNM::Timer, "NPCOnResponse");
        ComLogDebug("on bind method response", GenArg(Desc::methodId()), GenArg(m));
        MonitorTrace(runtime::GetInstance(), trace::userdefine::kRecvResponse, trace::ServiceType::kProxy,
        ...
        if (threadPool != nullptr) {
            threadPool->Dispatch([=] {
                ...
                std::ignore = thisObj->_handleResponse(std::shared_ptr< npc_message_t >(m, npc_message_close));
            });
            return 0;
        }
        auto* thisObj{reinterpret_cast< NpcMethod* >(ud)};
        return thisObj->_handleResponse(std::shared_ptr< npc_message_t >(m, npc_message_close));
    }
    /// @brief Handle call response message
    /// @param[in] response Response message
    /// @return Result -- 0: success; !0: failure;
    /// @ref [SWS_CM_10358]
    int32_t _handleResponse(std::shared_ptr< npc_message_t > const& response) noexcept
    {
        ...
        TimeStart(TimeNM::Timer, "NPCResponse::Verify&SecOC");
...
#else
#endif
        TimeStop(TimeNM::Timer, "NPCResponse::Verify&SecOC");
#if 0  // Source of error domain identifier not provided @ref [SWS_CM_10358]
    ...
    /// @brief Handle call response message -- with return value
    /// @tparam U Return type
    /// @param[in] response Response message
    /// @param[in] promise Promise of the associated call request
    /// @return Result -- 0: success; !0: failure;
    template < typename U = Output >
    int32_t _handleResponse(std::shared_ptr< npc_message_t > const& response,
                            ara::core::Promise< Output >& promise,
                            std::enable_if_t< !std::is_same< U, void >::value >* = nullptr) noexcept
    {
        TimeRecorder(TimeNM::Timer, "NPCHandleResponse(value)");
        Output value;
        int32_t ret{};
        if (e2eInfo_) {  // enable e2e
            serialize::E2EPayload payload{*response, e2eInfo_->dataId};
        ...
        ComLogDebug("handle bind method response: call bind method done", GenArg(value), GenArg(Desc::methodId()),
                    GenArg(response));
        MonitorTrace(runtime::GetInstance(), trace::userdefine::kDeserialize, trace::ServiceType::kProxy,
                     Desc::serviceId(), response->inst, Desc::serviceIdentifier, Desc::methodId(), "methodName", false,
                     response->hdr.session, static_cast< uint8_t >(trace::MsgType::kMethodResponse), 0U,
                     trace::FormatValue(value));
        promise.set_value(std::move(value));
        return 0;
    }
    /// @brief Handle call response message -- void return
    /// @tparam U Return type
    /// @param[in] response Response message
    /// @param[in] promise Promise of the associated call request
    /// @return Result -- 0: success; !0: failure;
    ...
    int32_t _handleResponse(std::shared_ptr< npc_message_t > const& response,
                            ...
                            std::enable_if_t< std::is_same< U, void >::value >* = nullptr) noexcept
    {
        TimeRecorder(TimeNM::Timer, "NPCHandleResponse(void)");
        int32_t ret{};
        if (e2eInfo_) {  // enable e2e
            serialize::E2EPayload payload{*response, e2eInfo_->dataId};
