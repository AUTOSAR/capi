...
#ifndef __COM_NPC_TYPES_H
#define __COM_NPC_TYPES_H

#include "ara/com/types.h"
#include "npc/core/npc_app.h"
#include "npc/core/npc_types.h"
#if 0  // Binding layer differences
    #include "npc/net/npc_message.h"
#else
    #include "npc/core/npc_message.h"
#endif

/// @brief Namespace -- internal binding layer types
namespace ara {
namespace com {
namespace internal {
namespace npc {
...
using ServiceId = npc_serv_t;
/// @brief Type alias -- instance identifier
using InstanceId = npc_inst_t;
/// @brief Type alias -- service major version
using ServiceVersionMajor = npc_version_t;
/// @brief Type alias -- service minor version
using ServiceVersionMinor = npc_versmin_t;
...
using EventGroupId = npc_gid_t;
/// @brief Type alias -- event identifier
using EventId = npc_eid_t;
/// @brief Type alias -- method identifier
using MethodId = npc_method_t;
...
/// @param[in] a Reference to original array
...
/// @tparam kN Number of array elements
/// @tparam kI List of array element indices
/// @param[in] a Rvalue reference to original array
/// @return Generic fixed-length array
template < class T, size_t kN, size_t... kI >
constexpr ara::core::Array< std::remove_cv_t< T >, kN > ToArray(T(&&a)[kN], std::index_sequence< kI... >) noexcept
{
...
struct Message
{
    /// @brief Type alias -- implementation layer application
    using App = npc_app_t;
    /// @brief Type alias -- implementation layer message
    using Msg = npc_message_t;
    /// @brief Type alias -- service identifier
    ...
    using ClientId = npc_cid_t;
    /// @brief Type alias -- session identifier
    using SessionId = npc_session_t;
    ...
    static constexpr ServiceId kServiceIdAny{NPC_SERVICE_ANY};
    ...
    static constexpr InstanceId kInstanceIdAny{NPC_INSTANCE_ANY};

    /// @brief Constant -- service major version: default
    static constexpr MajorVersion kMajorVersionDefault{NPC_DEFAULT_MAJOR};
    /// @brief Constant -- service major version: any
    static constexpr MajorVersion kMajorVersionAny{NPC_MAJOR_ANY};
    ...
    static constexpr MinorVersion kMinorVersionAny{NPC_MINOR_ANY};
    ...
    static constexpr MethodId kMethodIdAny{NPC_METHOD_ANY};
    ...
    static constexpr ProtocolVersion kProtocolVersionDefault{NPC_PROTOCOL_VERSION};
    ...
    static constexpr MessageType kMessageTypeRequest{NPC_MT_REQUEST};
    /// @brief Constant -- message type: 0X01 REQUEST_NO_RETURN A fire&forget request
    static constexpr MessageType kMessageTypeRequestNoReturn{NPC_MT_REQUEST_NO_RETURN};
    /// @brief Constant -- message type: 0X02 NOTIFICATION A request of a notification/event callback expecting no response
    static constexpr MessageType kMessageTypeNotification{NPC_MT_NOTIFICATION};
    /// @brief Constant -- message type: 0X80 RESPONSE The response message
    static constexpr MessageType kMessageTypeResponse{NPC_MT_RESPONSE};
    /// @brief Constant -- message type: 0X81 ERROR The response containing an error
    static constexpr MessageType kMessageTypeError{NPC_MT_ERROR};
    ...
    static constexpr TransportProtocol kTransportProtocolUnknown{NPC_RT_UNKNOWN};
    /// @brief Constant -- transport protocol: TCP
    static constexpr TransportProtocol kTransportProtocolTCP{NPC_RT_RELIABLE};
    /// @brief Constant -- transport protocol: UDP
    static constexpr TransportProtocol kTransportProtocolUDP{NPC_RT_UNRELIABLE};
    /// @brief Constant -- transport protocol: TCP/UDP
    static constexpr TransportProtocol kTransportProtocolBoth{NPC_RT_BOTH};
    ...
    App& app;
    /// @brief Implementation layer message
    std::shared_ptr< Msg > msg{};

    /// @brief Check if event identifier
    ...
    /// @brief Check if request identifier
    ...
    /// @brief Check if no-return request message
...
struct ServiceInfo
{
    ...
    /// @brief Check if P-side service information
    auto IsProvided() const noexcept { return type == Type::kProvided; }
    /// @brief Check if R-side service information
...
struct PServiceInfo : ServiceInfo
{
    ...
    struct SdServerConfig
    {
        ...
        struct InitialOfferBehavior
        {
            ...
            /// @brief Service discovery maximum number of repetitions
        ...
        /// @brief Allowed delay for responding to multicast requests
...
struct PGroupInfo : GroupInfo
{
    ...
    /// @brief Event group multicast threshold (no multicast address same as unicast)
...
struct RGroupInfo : GroupInfo
{
    ...
    {
        /// @brief Event group subscription timeout time (s)
...
struct PEventInfo : EventInfo
{
    ...
    /// @brief Data accumulation properties
    struct CollectionProps
    {
        ...
        {
            kAlways,  ///< Accumulation active
            kNever,   ///< Accumulation inactive
...
struct PMethodInfo : MethodInfo
{
    ...
    /// @brief Method data accumulation properties
    struct CollectionProps
    {
...
#if 0  // Binding layer differences
#else
/// @brief Log message cache
int32_t const kArrayMaxLength{1024};
#endif
}  // namespace npc
}  // namespace internal
}  // namespace com
...
namespace internal {
namespace format {
/// @brief Formatted message type
template <>
struct Formatter< ara::core::String, npc_message_t >
{
    /// @brief Convert to string
    /// @param[in, out] r String reference
    /// @param[in] m Message
    /// @return String reference
    static ara::core::String& ToString(ara::core::String& r, npc_message_t const& m) noexcept
    {
        return ara::com::internal::npc::types::ToString(r, m);
    }
};
}  // namespace format
}  // namespace internal
