...
#include "nai/os/nai_stat.h"
    ...
    #include "npc/core/npc_routing.h"
#else
#endif
#ifdef HAS_COM_SECOC
...
namespace internal {
namespace npc {
/// @brief Flag indicating whether it is a daemon process
bool g_IsDaemon{false};
/// @brief Binding layer runtime instance pointer
std::unique_ptr< NpcRuntime > g_NpcRuntime{};  // NOLINT[fuchsia-statically-constructed-objects]
ara::core::Result< void > Initialize(bool isDaemon) noexcept
{
    using Result = ara::core::Result< void >;
    ComLogTrace("initialize bind runtime", GenArg(isDaemon), GenArg(g_NpcRuntime));
    Result result{};
    assert(g_NpcRuntime == nullptr);
    g_IsDaemon   = isDaemon;
    g_NpcRuntime = std::make_unique< NpcRuntime >();
    if (auto res{g_NpcRuntime->Initialize()}) {
    } else {
        std::ignore = Deinitialize();
        result      = std::move(res);
        ComLogError("initialize bind runtime error", GenArg(result), GenArg(isDaemon), GenArg(g_NpcRuntime));
        return result;
    }
    ComLogDebug("initialize bind runtime done", GenArg(isDaemon), GenArg(g_NpcRuntime));
    return result;
}
ara::core::Result< void > Deinitialize() noexcept
{
    using Result = ara::core::Result< void >;
    ComLogTrace("deinitialize bind runtime", GenArg(g_NpcRuntime));
    Result result{};
    assert(g_NpcRuntime != nullptr);
    if (auto res{g_NpcRuntime->Deinitialize()}) {
    } else {
        result = std::move(res);
    }
    g_NpcRuntime = nullptr;
    ComLogDebug("deinitialize bind runtime done", GenArg(g_NpcRuntime));
    return result;
}
...
{
    static const ara::core::String kInstanceIdentifierPrefix{"NPC:"};
...
ara::core::Optional< ServiceInfo::InstanceId > ToNpcInstanceId(InstanceIdentifier const& input) noexcept
{
    ...
    try {
        ...
        // Failed to convert value, return empty
...
class NpcRuntime::Impl : public internal::BindRuntime
{
public:
    /// @brief Constructor
    Impl() noexcept
...
#if 0  // Binding layer differences
        NPC_ASSERT(npc_conf_init(&conf_), -__LINE__);
        NPC_ASSERT(npc_routing_init(&routing_), -__LINE__);
        NPC_ASSERT(npc_app_init(&app_), -__LINE__);
#else
        conf_ = NPC_ASSERT(npc_conf_init(), -__LINE__);
        app_  = NPC_ASSERT(npc_app_new(conf_), -__LINE__);
#endif
        if (!g_IsDaemon) {
            NPC_ASSERT(npc_appcb_init(&appcb_), -__LINE__);
        }
    }
    /// @brief Destructor
    ...
    ara::core::Result< void > Initialize() noexcept
    {
        ...
        Result result{};
        if (auto res{NPC_CHECK(InitializeBind(), -__LINE__)}) {
            deinitializes_.emplace_front([&] { return NPC_CHECK(DeinitializeBind(), -__LINE__); });
        } else {
            result = std::move(res);
            return result;
        }
        if (auto res{NPC_CHECK(InitializeNPC(), -__LINE__)}) {
            deinitializes_.emplace_front([&] { return NPC_CHECK(DeinitializeNPC(), -__LINE__); });
        } else {
            result = std::move(res);
            return result;
        }
        if (auto res{NPC_CHECK(InitializeE2E(), -__LINE__)}) {
            deinitializes_.emplace_front([&] { return NPC_CHECK(DeinitializeE2E(), -__LINE__); });
        } else {
            result = std::move(res);
            return result;
        }
#ifdef HAS_COM_SECOC
        if (auto res{NPC_CHECK(InitializeSecOC(), -__LINE__)}) {
            deinitializes_.emplace_front([&] { return NPC_CHECK(DeinitializeSecOC(), -__LINE__); });
        } else {
            result = std::move(res);
            return result;
        }
...
#endif
#ifdef HAS_ARA_IAM
        if (auto res{NPC_CHECK(InitializeIAM(), -__LINE__)}) {
            deinitializes_.emplace_front([&] { return NPC_CHECK(DeinitializeIAM(), -__LINE__); });
        } else {
            result = std::move(res);
            return result;
        }
    ...
    npc_app_t* GetApp() noexcept { return &app_; }
#else
    /// @brief Get implementation layer application
    /// @return Implementation layer application pointer
    npc_app_t* GetApp() noexcept { return app_; }
    ...
    npc_appcb_t* GetAppCB() noexcept { return &appcb_; }
    ...
    void RegisterProxyMapper(std::shared_ptr< ProxyMapperBase > const& mapper) noexcept
    {
        ...
        NPC_ASSERT(mapper1 == nullptr && mapper2 == nullptr, "register proxy mapper repeat");
    ...
    void RegisterSkeletonMapper(std::shared_ptr< SkeletonMapperBase > const& mapper) noexcept
    {
        ...
        NPC_ASSERT(mapper1 == nullptr && mapper2 == nullptr, "register skeleton mapper repeat");
        mapper1 = mapper2 = mapper;
    }
...
public:
    ...
    ara::core::Result< void > InitializeBind() noexcept
    {
        ...
        if (!siManifestRes) {
            // Ignore here, initializing the communication runtime maps instance descriptors and instance identifier sets. If opening fails here, it only means the application does not need instance deployment configuration.
            ...
            if (key3 == "name") {
                ...
                std::ignore = node3.IterateObject("", [&](auto& key4, auto& node4) {
                    ...
                    if (key4 == "initial_offer_behavior") {
                        ...
                        // [TODO] Check impact on platform startup, unblock after implementer is implemented
    ...
    /// @return Result object -- empty/value or error
    ara::core::Result< void > InitializeNPC() noexcept
    {
        using Result = ara::core::Result< void >;
        ...
        isoft::ara_fsh::Process proc;
        auto manifestFile{proc.GetConfig("npc.json")};
        // constexpr auto manifestFile{"etc/npc.json"};
        ...
        Loader loader{[&](auto& key, auto& node) {
            ...
            if (node.IsObject()) {
                if (!key.empty()) {
                    std::ignore = npc_conf_setopts(&conf_, "begin", key.data());
                }
                std::ignore = node.IterateObject("", loader);
                if (!key.empty()) {
                    std::ignore = npc_conf_setopts(&conf_, "end", "");
                }
                ...
                std::ignore = npc_conf_setopts(&conf_, key.data(), value.c_str());
            } else if (node.IsInt()) {
                int32_t value{};
                ...
                std::ignore = npc_conf_setopti(&conf_, key.data(), value);
        ...
        ret = NPC_CHECK(npc_conf_finish(&conf_), -__LINE__);
        if (ret < 0) {
            result.EmplaceError(MakeErrorCode(ComErrc::kCommunicationStackError, -__LINE__));
            return result;
        }
        ret = NPC_CHECK(npc_routing_set_conf(&routing_, &conf_), -__LINE__);
        if (ret < 0) {
            result.EmplaceError(MakeErrorCode(ComErrc::kCommunicationStackError, -__LINE__));
            return result;
        }
        ...
        npc_string_t skey;
    #undef npc_string
    #define npc_string(s) {reinterpret_cast< unsigned char* >(const_cast< char* >(s)), sizeof(s) - 1, 1, 1}
        npc_string_t appid = npc_string("appid");
        npc_string_t shmSize = npc_string("shm_size");
        npc_string_t host = npc_string("host");
        npc_string_t mode = npc_string("mode");
        npc_string_t svcNetType = npc_string("svc_net_type");
        npc_string_t svcAccessHandle = npc_string("svc_access_handle");
        npc_string_t disNetType = npc_string("dis_net_type");
        npc_string_t disAccessHandle = npc_string("dis_access_handle");
        npc_string_t sendTimeout = npc_string("send_timeout");
        npc_string_t nservice = npc_string("nservice");
        isoft::ara_fsh::Platform platform;
        ara::core::String dir{platform.GetUnixDomainSocketDir()};

        if (access(dir.c_str(), F_OK) != 0) {
            NPC_ASSERT(false, "initialize stack error: directory not exist: ", dir);
        }

        i = manifest->GetValue< int32_t >(reinterpret_cast< char* >(appid.data), -1);
        std::ignore = npc_conf_update(conf_, cpi_conf_type_num, &appid, &i);

        i = manifest->GetValue< int32_t >(reinterpret_cast< char* >(shmSize.data), -1);
        std::ignore = npc_conf_update(conf_, cpi_conf_type_num, &shmSize, &i);

        ara::core::String str{manifest->GetValue< ara::core::String >(reinterpret_cast< char* >(host.data), dir)};
        npc_string_set(&s, str.c_str());  // NOLINT -- compatibility >[cppcoreguidelines-pro-type-cstyle-cast]
        std::ignore = npc_conf_update(conf_, cpi_conf_type_string, &host, &s);

        str = manifest->GetValue< ara::core::String >(reinterpret_cast< char* >(mode.data), "");
        npc_string_set(&s, str.c_str());  // NOLINT -- compatibility >[cppcoreguidelines-pro-type-cstyle-cast]
        std::ignore = npc_conf_update(conf_, cpi_conf_type_string, &mode, &s);

        str = manifest->GetValue< ara::core::String >(reinterpret_cast< char* >(svcNetType.data), "");
        npc_string_set(&s, str.c_str());  // NOLINT -- compatibility >[cppcoreguidelines-pro-type-cstyle-cast]
        std::ignore = npc_conf_update(conf_, cpi_conf_type_string, &svcNetType, &s);

        str = manifest->GetValue< ara::core::String >(reinterpret_cast< char* >(svcAccessHandle.data), "");
        npc_string_set(&s, str.c_str());  // NOLINT -- compatibility >[cppcoreguidelines-pro-type-cstyle-cast]
        std::ignore = npc_conf_update(conf_, cpi_conf_type_string, &svcAccessHandle, &s);

        str = manifest->GetValue< ara::core::String >(reinterpret_cast< char* >(disNetType.data), "");
        npc_string_set(&s, str.c_str());  // NOLINT -- compatibility >[cppcoreguidelines-pro-type-cstyle-cast]
        std::ignore = npc_conf_update(conf_, cpi_conf_type_string, &disNetType, &s);

        str = manifest->GetValue< ara::core::String >(reinterpret_cast< char* >(disAccessHandle.data), "");
        npc_string_set(&s, str.c_str());  // NOLINT -- compatibility >[cppcoreguidelines-pro-type-cstyle-cast]
        std::ignore = npc_conf_update(conf_, cpi_conf_type_string, &disAccessHandle, &s);

        i = manifest->GetValue< int32_t >(reinterpret_cast< char* >(sendTimeout.data), 0);
        std::ignore = npc_conf_update(conf_, cpi_conf_type_num, &sendTimeout, &i);

        i = manifest->GetValue< int32_t >(reinterpret_cast< char* >(nservice.data), -1);
        std::ignore = npc_conf_update(conf_, cpi_conf_type_num, &nservice, &i);

        for (j = 0; j < i; ++j) {
            n = snprintf(key, (sizeof(key)) - 1, "service[%d].service", j);
            key[n] = 0;  // NOLINT[cppcoreguidelines-pro-bounds-constant-array-index]
            k = manifest->GetValue< int32_t >(key, -1);
            n = snprintf(key, (sizeof(key)) - 1, "service.%d.service", j);
            key[n] = 0;                  // NOLINT[cppcoreguidelines-pro-bounds-constant-array-index]
            npc_string_set(&skey, key);  // NOLINT -- compatibility >[cppcoreguidelines-pro-type-cstyle-cast]
            std::ignore = npc_conf_update(conf_, cpi_conf_type_num, &skey, &k);

            n = snprintf(key, (sizeof(key)) - 1, "service[%d].instance", j);
            key[n] = 0;  // NOLINT[cppcoreguidelines-pro-bounds-constant-array-index]
            k = manifest->GetValue< int32_t >(key, -1);
            n = snprintf(key, (sizeof(key)) - 1, "service.%d.instance", j);
            key[n] = 0;                  // NOLINT[cppcoreguidelines-pro-bounds-constant-array-index]
            ...
            n = snprintf(key, (sizeof(key)) - 1, "service[%d].major", j);
            key[n] = 0;  // NOLINT[cppcoreguidelines-pro-bounds-constant-array-index]
            k = manifest->GetValue< int32_t >(key, -1);
            n = snprintf(key, (sizeof(key)) - 1, "service.%d.major", j);
            key[n] = 0;                  // NOLINT[cppcoreguidelines-pro-bounds-constant-array-index]
            ...
            n = snprintf(key, (sizeof(key)) - 1, "service[%d].minor", j);
            key[n] = 0;  // NOLINT[cppcoreguidelines-pro-bounds-constant-array-index]
            k = manifest->GetValue< int32_t >(key, -1);
            n = snprintf(key, (sizeof(key)) - 1, "service.%d.minor", j);
            key[n] = 0;                  // NOLINT[cppcoreguidelines-pro-bounds-constant-array-index]
            ...
            n = snprintf(key, (sizeof(key)) - 1, "service[%d].shm_path", j);
            key[n] = 0;  // NOLINT[cppcoreguidelines-pro-bounds-constant-array-index]
            str = manifest->GetValue< ara::core::String >(key, "");
            n = snprintf(key, (sizeof(key)) - 1, "service.%d.shm_path", j);
            key[n] = 0;                       // NOLINT[cppcoreguidelines-pro-bounds-constant-array-index]
            npc_string_set(&skey, key);       // NOLINT -- compatibility >[cppcoreguidelines-pro-type-cstyle-cast]
            npc_string_set(&s, str.c_str());  // NOLINT -- compatibility >[cppcoreguidelines-pro-type-cstyle-cast]
            std::ignore = npc_conf_update(conf_, cpi_conf_type_string, &skey, &s);

            n = snprintf(key, (sizeof(key)) - 1, "service[%d].shm_seg_size", j);
            key[n] = 0;  // NOLINT[cppcoreguidelines-pro-bounds-constant-array-index]
            k = manifest->GetValue< int32_t >(key, -1);
            n = snprintf(key, (sizeof(key)) - 1, "service.%d.shm_seg_size", j);
            key[n] = 0;                  // NOLINT[cppcoreguidelines-pro-bounds-constant-array-index]
            ...
            n = snprintf(key, (sizeof(key)) - 1, "service[%d].shm_seg_count", j);
            key[n] = 0;  // NOLINT[cppcoreguidelines-pro-bounds-constant-array-index]
            k = manifest->GetValue< int32_t >(key, -1);
            n = snprintf(key, (sizeof(key)) - 1, "service.%d.shm_seg_count", j);
            key[n] = 0;                  // NOLINT[cppcoreguidelines-pro-bounds-constant-array-index]
            ...
            n = snprintf(key, (sizeof(key)) - 1, "service[%d].msg_queue_len", j);
            key[n] = 0;  // NOLINT[cppcoreguidelines-pro-bounds-constant-array-index]
            k = manifest->GetValue< int32_t >(key, -1);
            n = snprintf(key, (sizeof(key)) - 1, "service.%d.msg_queue_len", j);
            key[n] = 0;                  // NOLINT[cppcoreguidelines-pro-bounds-constant-array-index]
...
#if 0  // Binding layer differences
        ret = NPC_CHECK(npc_routing_open(&routing_, evloop_->GetRawEvLoop(), NPC_RTYPE_AUTO), -__LINE__);
        if (ret < 0) {
            result.EmplaceError(MakeErrorCode(ComErrc::kCommunicationStackError, -__LINE__));
            return result;
        }
#else
        char msg[kArrayMaxLength]{};
        npc_app_set_evloop(app_, evloop_->GetRawEvLoop());
#endif
        // Open implementation layer application
#if 0  // Binding layer differences
        auto& comRuntime{internal::GetInstance()};
        ret = NPC_CHECK(npc_app_open_name(&app_, &routing_, comRuntime.GetProcessName().data()), -__LINE__);
#else
        auto rc{npc_app_open(app_)};
        ret = NPC_CHECK(rc, npc_error(rc, msg, sizeof(msg)));
#endif
        if (ret < 0) {
            result.EmplaceError(MakeErrorCode(ComErrc::kCommunicationStackError, -__LINE__));
            return result;
        ...
        if (!g_IsDaemon) {
#if 0  // Binding layer differences
            ret = NPC_CHECK(npc_appcb_open(&appcb_, &app_), -__LINE__);
#else
            rc = npc_appcb_open(&appcb_, app_);
            ret = NPC_CHECK(rc, npc_error(rc, msg, sizeof(msg)));
#endif
            if (ret < 0) {
                result.EmplaceError(MakeErrorCode(ComErrc::kCommunicationStackError, -__LINE__));
    ...
    /// @return Result object -- empty/value or error
    ara::core::Result< void > DeinitializeNPC() noexcept
    {
        using Result = ara::core::Result< void >;
        ComLogTrace("deinitialize stack", GenArg(this));
        Result result{};
        // Ignore implementation layer deinitialization (uninitialized)
        if (evloop_ == nullptr) {
        ...
        int32_t ret{};
        if (!g_IsDaemon) {
            ret = NPC_CHECK(npc_appcb_close(&appcb_), -__LINE__);
            if (ret < 0) {
                result.EmplaceError(MakeErrorCode(ComErrc::kCommunicationStackError, -__LINE__));
            }
        }
...
#if 0  // Binding layer differences
        ret = NPC_CHECK(npc_app_close(&app_), -__LINE__);
        if (ret < 0) {
            result.EmplaceError(MakeErrorCode(ComErrc::kCommunicationStackError, -__LINE__));
        }
#else
...
#if 0  // Binding layer differences
        ret = NPC_CHECK(npc_routing_close(&routing_), -__LINE__);
        if (ret < 0) {
            result.EmplaceError(MakeErrorCode(ComErrc::kCommunicationStackError, -__LINE__));
        }
#else
...
#if 0  // Binding layer differences
        ret = NPC_CHECK(npc_conf_close(&conf_), -__LINE__);
        if (ret < 0) {
            result.EmplaceError(MakeErrorCode(ComErrc::kCommunicationStackError, -__LINE__));
        }
#else
        std::ignore = ret;
        npc_conf_close(conf_);
#endif
        // Unbind event loop
        evloop_ = nullptr;
        return result;
    ...
    ara::core::Result< void > InitializeIAM() noexcept
    {
        ...
        if (!g_IsDaemon) {
            ...
            if (auto res{ara::iam::internal::grant::IAMGrantComQuery::Initialize()}) {
                npc_app_set_cb_iam_subscribe(app_, Impl::OnIAMSubscribe);
                npc_app_set_cb_iam_message(app_, Impl::OnIAMMessage);
            ...
            if (auto res{ara::iam::internal::grant::IAMGrantComQuery::Initialize()}) {
    #if 0  // Binding layer differences
                npc_app_set_cb_offer(&app_, Impl::OnIAMOfferService);
                npc_app_set_cb_find(&app_, Impl::OnIAMFindService);
                npc_app_set_cb_subscribe(&app_, Impl::OnIAMSubscribe);
                npc_app_set_cb_message(&app_, Impl::OnIAMMessage);
    #else
                npc_app_set_cb_offer(app_, Impl::OnIAMOfferService);
                npc_app_set_cb_find(app_, Impl::OnIAMFindService);
    #endif
    ...
    #if 0  // Binding layer differences
            npc_app_set_cb_offer(&app_, nullptr);
            npc_app_set_cb_find(&app_, nullptr);
            npc_app_set_cb_subscribe(&app_, nullptr);
            npc_app_set_cb_message(&app_, nullptr);
    #else
            npc_app_set_cb_offer(app_, nullptr);
            npc_app_set_cb_find(app_, nullptr);
    #endif
...
private:
    ...
    npc_appcb_t appcb_{};
    /// @brief Discovery service handle callback data type
    struct FindHandleCBData
    {
        /// @brief Discovery service handler
    ...
    /// @brief Discovery service handle comparator
    ...
    /// @brief Type alias -- discovery service handle to callback data multimap
    using FindHandle2CBData
        = std::multimap< FindServiceHandle, std::shared_ptr< FindHandleCBData >, FindHandleComparer >;
    /// @brief Discovery service handle to callback data multimap
    ...
    /// @brief Request reference count for service instance
    ara::core::Map< ServiceInfo::InstanceUID, size_t > requestedServices_;
    /// @brief Mutex -- for requesting service
    std::mutex requestedServicesMutex_;
    /// @brief Event group information
    struct GroupInfo
    {
        /// @brief Event group identifier to event identifier set map
        ara::core::Map< npc_gid_t, ara::core::Vector< npc_eid_t > > gid2Ids;
        /// @brief Event identifier to subscription event confirmation callback map
        ara::core::Map< npc_eid_t, std::function< void(int32_t rcode) > > ackId2Listener;
    };
    /// @brief Instance key to event group information map
    ara::core::Map< ServiceInfo::InstanceID, GroupInfo > activeGroups_;
    /// @brief Mutex -- for event group
...
public:
...
#if 0  // Binding layer differences
        auto ret{NPC_CHECK(npc_app_offer_service(&app_, std::get< 0 >(instanceUID), std::get< 1 >(instanceUID),
...
#else
        char msg[kArrayMaxLength];
        auto rc{npc_app_offer_service(app_, std::get< 0 >(instanceUID), std::get< 1 >(instanceUID),
                                      std::get< 2 >(instanceUID), std::get< 3 >(instanceUID))};
        auto ret{NPC_VERIFY(rc, npc_error(rc, msg, sizeof(msg)), GenArg(instanceUID))};
#endif
        if (ret < 0) {
            return ret;
        }
        ret = NPC_CHECK(
            npc_appcb_set_subscribe(&appcb_, std::get< 0 >(instanceUID), std::get< 1 >(instanceUID), Impl::OnSubscribe),
    ...
    int32_t StopService(ServiceInfo::InstanceUID const& instanceUID) noexcept
    {
        ...
        int32_t result{};
        auto ret{
            NPC_CHECK(npc_appcb_set_subscribe(&appcb_, std::get< 0 >(instanceUID), std::get< 1 >(instanceUID), nullptr),
                      "stop stack service error: invoke stack unset subscribe callback failed", GenArg(instanceUID))};
        if (ret < 0) {
            result = ret;
        }
#if 0  // Binding layer differences
        ret = NPC_CHECK(npc_app_stop_service(&app_, std::get< 0 >(instanceUID), std::get< 1 >(instanceUID),
                                             std::get< 2 >(instanceUID), std::get< 3 >(instanceUID)),
                        "stop stack service error: invoke stack stop service failed", GenArg(instanceUID));
#else
        char msg[kArrayMaxLength];
        auto rc{npc_app_stop_service(app_, std::get< 0 >(instanceUID), std::get< 1 >(instanceUID),
                                     std::get< 2 >(instanceUID), std::get< 3 >(instanceUID))};
        ret = NPC_VERIFY(rc, npc_error(rc, msg, sizeof(msg)), GenArg(instanceUID));
#endif
        if (ret < 0) {
            result = ret;
        }
...
private:
    ...
    /// @return Result -- 0: success; !0: failure;
    static int32_t OnIAMOfferService(npc_app_t* a,
                                     void* ud,
                                     npc_serv_t serv,
                                     npc_inst_t inst,
                                     npc_version_t major,
                                     npc_versmin_t minor,
                                     npc_cred_t const* cred) noexcept
    {
        std::ignore = a;
        std::ignore = ud;
        std::ignore = major;
...
public:
                                        ...
                                        bindHandles) noexcept override
    {
        ...
        for (auto it{rMappers.begin()}; it != rMappers.end(); ++it) {
            ...
            // Trigger a query, callback destruction will automatically end the query
            ServiceRequestedHolder requestedHolder{
                ServiceInfo::InstanceUID{serviceId, instanceId, requestMajor, requestMinor}};
            // Get active service instances (from implementation layer cache, based on given service/instance information, and compatible version)
            nai_array_t out;
            auto ret{NPC_CHECK(nai_array_init(&out, sizeof(npc_service_info_t), nullptr), -__LINE__)};
            if (ret < 0) {
                continue;
            }
            ret = NPC_CHECK(npc_appcb_get_availables(&appcb_, serviceId, instanceId, &out), GenArg(serviceId),
                            GenArg(instanceId));
            if (ret < 0) {
                std::ignore = nai_array_close(&out);
                continue;
            }
            auto* availableInfo{reinterpret_cast< npc_service_info_t* >(out.elts)};
            ...
            std::ignore = nai_array_close(&out);
            {
                ...
                for (auto itAvailableInstanceInfo{availableInstanceInfos_.begin()};
                    ...
                    auto available{
                        npc_appcb_is_available(&appcb_, std::get< 0 >(instanceUID), std::get< 1 >(instanceUID))};
                    auto instanceAvailable{available == NPC_AVAIL_CHANGED || available == NPC_AVAIL_ONLINED};
    ...
    /// @brief Register discovery service handle -- for starting service discovery -- proxy side
    /// @param[in] findHandle Discovery service handle
    /// @param[in] findHandler Discovery service handler
                                       ...
                                       findHandler) noexcept override
    {
        ...
        auto optionalId{runtime::ToNpcInstanceId(findHandle.instanceIdentifier)};
        if (!optionalId) {  // Conversion failed, instance identifier not supported
        ...
        for (auto it{rMappers.begin()}; it != rMappers.end(); ++it) {
            ...
            // Synchronize this layer's callback data before registration (because the callback process involves both this layer and underlying layer locks, to ensure concurrency, avoid deadlock, and maintain query timing after registration, use smart pointer solution: reserve first, then construct to trigger query)
            ...
            if (enableSetAvailable) {
                NPC_ASSERT(npc_appcb_get_ud(&appcb_, serviceId, instanceId) == nullptr,
                           "register find service handle error: invoke stack set callback userdata repeat",
                           GenArg(serviceId), GenArg(instanceId));
                NPC_ASSERT(npc_appcb_set_ud(&appcb_, serviceId, instanceId, this),
                           "register find service handle error: invoke stack set callback userdata failed",
                           GenArg(serviceId), GenArg(instanceId));
                NPC_ASSERT(npc_appcb_set_available(&appcb_, serviceId, instanceId, Impl::OnServiceAvailable),
    ...
    /// @brief Unregister discovery service handle -- for stopping service discovery -- proxy side
    /// @param[in] findHandle Discovery service handle
    void UnregisterFindServiceHandle(FindServiceHandle const& findHandle) noexcept override
    {
        ...
        if (enableUnsetAvailable) {
            ...
            for (auto it{rMappers.begin()}; it != rMappers.end(); ++it) {
                ...
                auto serviceId{mapper->GetServiceId()};
                auto instanceId{*optionalId};
                NPC_CHECK(npc_appcb_get_ud(&appcb_, serviceId, instanceId) != nullptr,
                          "unregister find service handle error: invoke stack unset callback userdata repeat",
                          GenArg(serviceId), GenArg(instanceId));
                NPC_CHECK(npc_appcb_set_available(&appcb_, serviceId, instanceId, nullptr),
                          "unregister find service handle error: invoke stack unset callback failed", GenArg(serviceId),
                          GenArg(instanceId));
                NPC_CHECK(npc_appcb_set_ud(&appcb_, serviceId, instanceId, nullptr),
    ...
    void RequestService(ServiceInfo::InstanceUID const& instanceUID) noexcept
    {
        ...
        if (enableRequest) {
            ...
            NPC_ASSERT(npc_appcb_set_subscribe_ack(&appcb_, std::get< 0 >(instanceUID), std::get< 1 >(instanceUID),
                                                   Impl::OnSubscribeAck),
                       "request service error: invoke stack set subscribe ack callback failed", GenArg(instanceUID));
#if 0  // Binding layer differences
            NPC_ASSERT(npc_app_request_service(&app_, std::get< 0 >(instanceUID), std::get< 1 >(instanceUID),
                                               std::get< 2 >(instanceUID), std::get< 3 >(instanceUID)),
                       "request service error: invoke stack request service failed", GenArg(instanceUID));
#else
            char msg[kArrayMaxLength];
            auto rc{npc_app_request_service(app_, std::get< 0 >(instanceUID), std::get< 1 >(instanceUID),
                                            std::get< 2 >(instanceUID), std::get< 3 >(instanceUID))};
            NPC_ASSERT(rc, npc_error(rc, msg, sizeof(msg)), GenArg(instanceUID));
#endif
        }
    }
    /// @brief Release requested service -- proxy side
    /// @param[in] instanceUID Service instance unique identifier
    void ReleaseService(ServiceInfo::InstanceUID const& instanceUID) noexcept
    {
        ...
        auto enableRelease{false};
        {
            ...
            NPC_CHECK(it != requestedServices_.end(), "release service error: find requested services failed",
                      GenArg(instanceUID));
...
#if 0  // Binding layer differences
            NPC_CHECK(npc_app_release_service(&app_, std::get< 0 >(instanceUID), std::get< 1 >(instanceUID),
                                              std::get< 2 >(instanceUID), std::get< 3 >(instanceUID)),
                      "release service error: invoke stack release service failed", GenArg(instanceUID));
#else
            char msg[kArrayMaxLength];
            auto rc = npc_app_release_service(app_, std::get< 0 >(instanceUID), std::get< 1 >(instanceUID),
                                              std::get< 2 >(instanceUID), std::get< 3 >(instanceUID));
            NPC_VERIFY(rc, npc_error(rc, msg, sizeof(msg)), GenArg(instanceUID));
#endif
            NPC_CHECK(
                npc_appcb_set_subscribe_ack(&appcb_, std::get< 0 >(instanceUID), std::get< 1 >(instanceUID), nullptr),
...
private:
    ...
    /// @param[in] cred Credential information
    /// @return Result -- 0: success; !0: failure;
    static int32_t OnIAMFindService(npc_app_t* a,
                                    void* ud,
                                    npc_serv_t serv,
                                    npc_inst_t inst,
                                    npc_version_t major,
    ...
    /// @return Result -- 0: success; !0: failure;
#if 0  // Binding layer differences
    static int32_t OnServiceAvailable(npc_app_t* a,
                                      void* ud,
                                      int32_t avail,
                                      npc_serv_t serv,
                                      npc_inst_t inst,
...
#endif
    /// @brief Handle service availability change -- proxy side
    /// @param[in] instanceUID Service instance unique identifier
    ...
    int32_t _handleServiceAvailable(ServiceInfo::InstanceUID const& instanceUID, int32_t available) noexcept
    {
        ...
        // Update the list of effective (available and application-supported) services
        if (instanceAvailable) {
            std::lock_guard< Mutex > lock(availableInstanceInfosMutex_);
            auto itPair{availableInstanceInfos_.emplace(instanceUID)};
            if (!itPair.second) {
        ...
        // Get the currently application-supported service information from the registered service map via service identifier
        auto itMapper{rMappers_.find(std::get< 0 >(instanceUID))};
        NPC_ASSERT(itMapper != rMappers_.end(), GenArg(instanceUID));
        auto& mapper{itMapper->second};
        // Callback to the registered set of discovery service handles (associated discovery service handlers)
        ...
        for (auto it{rMappers.begin()}; it != rMappers.end(); ++it) {
            ...
            // Callback to discovery service handle set (specified instance identifier)
            ...
            if (serviceInfo != nullptr) {
                ...
                for (auto it2{itPair.first}; it2 != itPair.second;) {
                    ...
                    // Determine if the currently available service matches the required one
            ...
            // Callback to discovery service handle set (any instance identifier)
            ...
            // Callback to discovery service handle set (any binding instance identifier)
            auto anyNpcInstanceIdentifier{runtime::ToInstanceIdentifier(Message::kInstanceIdAny)};
            {
                std::lock_guard< Mutex > lock(findHandle2CBDataMutex_);
                auto itPair{findHandle2CBData_.equal_range({serviceIdentifier, anyNpcInstanceIdentifier, 0})};
                for (auto it2{itPair.first}; it2 != itPair.second;) {
                    auto itCurrent{it2++};
                    auto findServiceHandle{itCurrent->first};
                    auto findHandleCBData{itCurrent->second};
...
public:
                      ...
                      std::function< void(int32_t rcode) >&& callback) noexcept
    {
        ...
        NPC_CHECK(eid != Message::kMethodIdAny, "subscribe stack event refused: event id invalid", GenArg(serviceId),
...
#if 0  // Binding layer differences
        auto ret{NPC_CHECK(npc_app_request_event(&app_, serviceId, instanceId, eid, ev),
                           "subscribe stack event error: invoke stack request event failed", GenArg(serviceId),
                           GenArg(instanceId), GenArg(major), GenArg(eid))};
#else
        char msg[kArrayMaxLength];
        auto rc{npc_app_request_event(app_, serviceId, instanceId, eid, ev)};
        auto ret{NPC_ASSERT(rc, npc_error(rc, msg, sizeof(msg)), GenArg(serviceId), GenArg(instanceId), GenArg(eid))};
#endif
        if (ret < 0) {
            result = ret;
            return result;
...
#if 0  // Binding layer differences
        ret = NPC_CHECK(npc_app_subscribe(&app_, serviceId, instanceId, ev->groups[0], major, eid),
                        "subscribe stack event error: invoke stack subscribe failed", GenArg(serviceId), GenArg(instanceId),
                        GenArg(major), GenArg(eid));
#else
        rc = npc_app_subscribe(app_, serviceId, instanceId, ev->groups[0], major, eid);
        ret = NPC_ASSERT(rc, npc_error(rc, msg, sizeof(msg)), GenArg(serviceId), GenArg(instanceId), GenArg(major),
                         GenArg(eid));
#endif
        if (ret < 0) {
            result = ret;
                        ...
                        npc_event_info_t* ev) noexcept
    {
        ...
        NPC_CHECK(eid != Message::kMethodIdAny, "unsubscribe stack event refused: event id invalid", GenArg(serviceId),
                  GenArg(instanceId), GenArg(major), GenArg(eid));
        int32_t result{};
#if 0  // Binding layer differences
        auto ret{NPC_CHECK(npc_app_unsubscribe(&app_, serviceId, instanceId, ev->groups[0], major, eid),
                           "unsubscribe stack event error: invoke stack unsubscribe failed", GenArg(serviceId),
                           GenArg(instanceId), GenArg(major), GenArg(eid))};
#else
        char msg[kArrayMaxLength];
        auto rc{npc_app_unsubscribe(app_, serviceId, instanceId, ev->groups[0], major, eid)};
        auto ret{NPC_VERIFY(rc, npc_error(rc, msg, sizeof(msg)), GenArg(serviceId), GenArg(instanceId), GenArg(major),
                            GenArg(eid))};
#endif
        if (ret < 0) {
            result = ret;
...
#if 0  // Binding layer differences
        ret = NPC_CHECK(npc_app_release_event(&app_, serviceId, instanceId, eid),
                        "unsubscribe stack event error: invoke stack release event failed", GenArg(serviceId),
                        GenArg(instanceId), GenArg(major), GenArg(eid));
#else
        rc = npc_app_release_event(app_, serviceId, instanceId, eid);
        ret = NPC_VERIFY(rc, npc_error(rc, msg, sizeof(msg)), GenArg(serviceId), GenArg(instanceId), GenArg(eid));
#endif
        if (ret < 0) {
            result = ret;
        }
        {
            std::lock_guard< std::mutex > lock(activeGroupsMutex_);
            auto& groupInfo{activeGroups_[{serviceId, instanceId}]};
            ret = NPC_CHECK(groupInfo.ackId2Listener.erase(eid) == 1,
...
private:
    ...
    /// @param[in] cred Credential information
    /// @return Result -- 0: success; !0: failure;
    static int32_t OnIAMSubscribe(npc_app_t* a,
                                  void* ud,
                                  npc_cid_t cid,
                                  npc_serv_t serv,
                                  npc_inst_t inst,
                                  ...
                                  npc_cred_t const* cred) noexcept
    {
        ...
        switch (cred->type) {
            ...
            case NPC_CRED_ADDRESS:
                ...
                break;
            case NPC_CRED_LOCAL:  // NOLINT -- compatibility >[bugprone-branch-clone]
                break;
            case NPC_CRED_CERT:
                break;
            default:
    ...
    /// @param[in] cred Credential information
    /// @return Result -- 0: success; !0: failure;
    static int32_t OnSubscribe(npc_app_t* a,
                               void* ud,
                               npc_cid_t cid,
                               npc_serv_t serv,
                               npc_inst_t inst,
    ...
    /// @brief Handle subscription event -- skeleton side
    /// @param[in] cid Client identifier
    /// @param[in] serviceId Service identifier
    /// @param[in] instanceId Instance identifier
    /// @param[in] gid Event group identifier
                             ...
                             ServiceInfo::EventId eid) noexcept
    {
        ...
        MonitorTrace(runtime::GetInstance(), trace::userdefine::kEventSubscribed, trace::ServiceType::kSkeleton,
                     serviceId, instanceId, ara::com::internal::ServiceIdentifier("unkonw"), eid, "unkonw", false, 0);
#if 0  // Binding layer differences
        return NPC_CHECK(npc_app_subscribe_ack(&app_, Message::kReturnCodeOk, cid, serviceId, instanceId, gid, eid),
...
#else
        char msg[kArrayMaxLength];
        auto rc{npc_app_subscribe_ack(app_, Message::kReturnCodeOk, cid, serviceId, instanceId, gid, eid)};
        return NPC_VERIFY(rc, npc_error(rc, msg, sizeof(msg)), GenArg(serviceId), GenArg(instanceId), GenArg(gid),
                          GenArg(eid));
#endif
    }
    /// @brief Receive subscription event confirmation -- proxy side
    /// @param[in] a Implementation layer application
    /// @param[in] ud User data
    ...
    static int32_t OnSubscribeAck(
        npc_app_t* a, void* ud, int32_t rcode, npc_serv_t serv, npc_inst_t inst, npc_gid_t gid, npc_eid_t eid) noexcept
    {
        std::ignore = a;
        std::ignore = ud;
    ...
    /// @brief Handle subscription event confirmation -- proxy side
    /// @param[in] rcode Return code
    /// @param[in] serviceId Service identifier
    /// @param[in] instanceId Instance identifier
    /// @param[in] gid Event group identifier
...
public:
    ...
    /// @param[in] m Message
    /// @return Result -- 0: success; !0: failure;
    int32_t SendMessage(npc_message_t* m) noexcept
    {
        TimeRecorder(TimeNM::Timer, "NPCSendMessage");
        ComLogTrace("send message", GenArg(m));
#if 0  // Binding layer differences
        auto ret{NPC_CHECK(npc_app_send(&app_, m), "send message error: invoke stack send failed", GenArg(m))};
#else
        char msg[kArrayMaxLength];
        auto rc{npc_app_send(app_, m)};
        auto ret{NPC_VERIFY(rc, npc_error(rc, msg, sizeof(msg)), GenArg(m))};
#endif
        if (ret < 0) {
            std::ignore = npc_message_close(m);
        }
        return ret;
    }
...
private:
    ...
    /// @param[in] m Message
    /// @return Result -- 0: success; !0: failure;
    static int32_t OnIAMMessage(npc_app_t* a, void* ud, npc_message_t* m) noexcept
    {
        std::ignore = a;
        std::ignore = ud;
...
NpcRuntime::ProxyMapperBase::ProxyMapperBase() noexcept { ComLogTrace(""); }
NpcRuntime::ProxyMapperBase::~ProxyMapperBase() noexcept { ComLogTrace(""); }
NpcRuntime::SkeletonMapperBase::SkeletonMapperBase() noexcept { ComLogTrace(""); }
NpcRuntime::SkeletonMapperBase::~SkeletonMapperBase() noexcept { ComLogTrace(""); }

NpcRuntime::ServiceRequestedHolder::ServiceRequestedHolder(ServiceInfo::InstanceUID instanceUIDIn) noexcept
    : instanceUID{std::move(instanceUIDIn)}
{
    auto& bindRuntime{runtime::GetInstance()};
    bindRuntime.impl_->RequestService(instanceUID);
}
NpcRuntime::ServiceRequestedHolder::~ServiceRequestedHolder() noexcept
{
    auto& bindRuntime{runtime::GetInstance()};
    bindRuntime.impl_->ReleaseService(instanceUID);
}

NpcRuntime::NpcRuntime() noexcept { ComLogTrace("create bind runtime"); }
NpcRuntime::~NpcRuntime() noexcept { ComLogTrace("destroy bind runtime"); }
ara::core::Result< void > NpcRuntime::Initialize() noexcept
{
    using Result = ara::core::Result< void >;
...
ara::core::Result< void > NpcRuntime::Deinitialize() noexcept
{
    using Result = ara::core::Result< void >;
...
void NpcRuntime::_registerProxyMapper(std::shared_ptr< ProxyMapperBase > const& mapper) noexcept
{
    impl_->RegisterProxyMapper(mapper);
}
void NpcRuntime::_registerSkeletonMapper(std::shared_ptr< SkeletonMapperBase > const& mapper) noexcept
{
    impl_->RegisterSkeletonMapper(mapper);
}
ServiceInfo* NpcRuntime::GetPServiceInfo(decltype(ServiceInfo::serviceId) serviceId,
                                         decltype(ServiceInfo::instanceId) instanceId) const noexcept
{
    return impl_->GetPServiceInfo(serviceId, instanceId);
}
ServiceInfo* NpcRuntime::GetRServiceInfo(decltype(ServiceInfo::serviceId) serviceId,
                                         decltype(ServiceInfo::instanceId) instanceId) const noexcept
{
    return impl_->GetRServiceInfo(serviceId, instanceId);
}
npc_app_t* NpcRuntime::GetApp() noexcept { return impl_->GetApp(); }
npc_appcb_t* NpcRuntime::GetAppCB() noexcept { return impl_->GetAppCB(); }
int32_t NpcRuntime::OfferService(ServiceInfo::InstanceUID const& instanceUID) noexcept
{
    return impl_->OfferService(instanceUID);
}
int32_t NpcRuntime::StopService(ServiceInfo::InstanceUID const& instanceUID) noexcept
{
    return impl_->StopService(instanceUID);
}
int32_t NpcRuntime::SubscribeEvent(ServiceInfo::ServiceId serviceId,
                                   ServiceInfo::InstanceId instanceId,
                                   ServiceInfo::MajorVersion major,
                                   ServiceInfo::EventId eid,
                                   npc_event_info_t* ev,
...
int32_t NpcRuntime::UnsubscribeEvent(ServiceInfo::ServiceId serviceId,
                                     ServiceInfo::InstanceId instanceId,
                                     ServiceInfo::MajorVersion major,
                                     ServiceInfo::EventId eid,
                                     npc_event_info_t* ev) noexcept
...
int32_t NpcRuntime::Send(npc_message_t* m) noexcept { return impl_->SendMessage(m); }
}  // namespace npc
}  // namespace internal
}  // namespace com
}  // namespace ara