...
#ifndef __COM_ICC_SIGNAL_H
#define __COM_ICC_SIGNAL_H
...
/// @brief Template type -- check if based on signal (no)
...
/// @brief Template type -- check if based on signal (yes)
/// @tparam Desc Description information
template < typename Desc >
...
/// @brief Check if based on signal
/// @tparam Desc Description information
/// @return bool
template < typename Desc >
...
/// @brief Template type -- check if signal service (no)
/// @tparam T Description information
/// @tparam Tagged Tag type for SFINAE
template < typename T, typename Tagged = void >
...
/// @brief Template type -- check if signal service (yes)
/// @tparam Desc Description information
template < typename Desc >
...
/// @brief Template type -- check if SOME/IP service (no)
/// @tparam T Description information
/// @tparam Tagged Tag type for SFINAE
template < typename T, typename Tagged = void >
...
/// @brief Template type -- check if SOME/IP service (yes)
/// @tparam Desc Description information
template < typename Desc >
...
struct S2SProps
{
    ...
    /// @details Used to establish the fan-in/fan-out and trigger implementation associated with this association (specialized source/target PDU header identifier pair)
...
/// @brief
namespace details {
/// @brief Template type -- check if source S2S
/// @tparam id PDU header identifier
...
{
    /// @brief Check if source PDU header identifier exists -- list has multiple values
    enum
    {
        kValue = id == sourceId || IsS2SSource< id, HeaderIds< sourceIds... > >::kValue
    };
};
/// @brief Template type -- check if target S2S
/// @tparam id PDU header identifier
...
{
    /// @brief Check if target PDU header identifier exists -- list has multiple values
    enum
    {
