...
#ifndef __COM_ICC_SERIALIZE_STREAM_H
#define __COM_ICC_SERIALIZE_STREAM_H

#include <list>

#include "icc_serialize_base.h"
#include "icc_types.h"
...
template < typename TProps, typename TPayload, typename = void >
class StreamReader;
/// @brief Template type -- payload stream writer -- specialization for null pointer type (used for calculating written byte count)
/// @tparam TProps Property information
...
/// @tparam TProps Property information
template < typename TProps >
class StreamWriter< TProps, icc_message_t >
{
public:
    /// @brief Type alias -- property information
    using PropsType = TProps;
    /// @brief Type alias -- payload type
    using PayloadType = icc_message_t;
    /// @brief Type alias -- result type
    using ResultType = int32_t;
    /// @brief Type alias -- position type
    using PointType = icc_message_io_t;
    /// @brief Constructor
    /// @param[in] payload Payload
    /// @param[in] capacity Payload capacity -- pre-allocation
    explicit StreamWriter(PayloadType& payload, size_t capacity) noexcept : payload_{payload}
    {
        std::ignore = icc_message_write_start(&point_, &payload_);
#if 0  // Binding layer differences
        std::ignore = icc_message_reserve(&point_, capacity);
#else
        std::ignore = capacity;
#endif
    }
    /// @brief Destructor
    ~StreamWriter() noexcept { std::ignore = icc_message_write_end(&point_); }
    /// @brief Copy constructor
    /// @param other
    ...
    inline ResultType Write(TValue const& value, bool reverseOrder) noexcept
    {
        ...
        if (reverseOrder && kSize != 1) {
            uint8_t data1[kSize];
            std::ignore = std::reverse_copy(data, data + kSize, data1);
            if (static_cast< size_t >(icc_message_write(&point_, data1, kSize)) != kSize) {
                return ErrorCode::kInvalidWrite;  // Write exceeds payload capacity limit (value)
            }
        } else {
            if (static_cast< size_t >(icc_message_write(&point_, data, kSize)) != kSize) {
                return ErrorCode::kInvalidWrite;  // Write exceeds payload capacity limit (value)
            }
        }
        return static_cast< ResultType >(kSize);
    ...
    inline ResultType Write(TValue const& value, bool reverseOrder, PointType const& point) noexcept
    {
        ...
        if (reverseOrder && kSize != 1) {
            uint8_t data1[kSize];
            std::ignore = std::reverse_copy(data, data + kSize, data1);
            if (icc_message_overwrite(const_cast< PointType* >(&point), data1, kSize) != kSize) {
                return ErrorCode::kInvalidWrite;  // Write exceeds payload capacity limit (value)
            }
        } else {
            if (icc_message_overwrite(const_cast< PointType* >(&point), data, kSize) != kSize) {
                return ErrorCode::kInvalidWrite;  // Write exceeds payload capacity limit (value)
            }
        }
        return static_cast< ResultType >(kSize);
...
/// @tparam TProps Property information
template < typename TProps >
class StreamReader< TProps, icc_message_t >
{
public:
    /// @brief Type alias -- property information
    using PropsType = TProps;
    /// @brief Type alias -- payload type
    using PayloadType = icc_message_t const;
    /// @brief Type alias -- result type
    using ResultType = int32_t;
    /// @brief Type alias -- position type
    using PointType = icc_message_io_t;
    ...
    explicit StreamReader(PayloadType& payload) noexcept : payload_{payload}
    {
        std::ignore = icc_message_read_start(&point_, const_cast< icc_message_t* >(&payload_), 0);
    }
    /// @brief Destructor
    ~StreamReader() noexcept { std::ignore = icc_message_read_end(&point_); }
    /// @brief Copy constructor
    /// @param other
    ...
    inline ResultType Read(TValue& value, bool reverseOrder) noexcept
    {
        ...
        if (static_cast< size_t >(icc_message_read(&point_, static_cast< void* >(data), kSize)) != kSize) {
    ...
    inline ResultType Read(TValue& value, bool reverseOrder, PointType const& point) const noexcept
    {
        ...
        if (static_cast< size_t >(
                icc_message_read(const_cast< icc_message_io_t* >(&point), static_cast< void* >(data), kSize))
            != kSize) {
            return ErrorCode::kInvalidRead;
        }
        if (reverseOrder && kSize != 1) {
...
public:
    ...
    E2EPayload(icc_message_t const& message, ara::com::e2exf::IdlistType dataId) noexcept
        : m{const_cast< icc_message_t& >(message)}, id{std::move(dataId)}
    {
    }
    /// @brief Message
    icc_message_t& m;
...
public:
    ...
    ~StreamWriter() noexcept
    {
        ...
        std::ignore = icc_message_write_start(&point_.m, &payload_.m);
        auto data{payload_.v.data() + point_.offset};
        auto kSize{payload_.v.size() - point_.offset};
        if (static_cast< size_t >(icc_message_write(&point_.m, data, kSize)) != kSize) {
            ComLogError("");  // Write exceeds payload capacity limit
        }
        std::ignore = icc_message_write_end(&point_.m);
    }
    /// @brief Copy constructor
    /// @param other
    StreamWriter(StreamWriter const& other) noexcept = default;
...
public:
    ...
    explicit StreamReader(PayloadType const& payload) noexcept : payload_{const_cast< PayloadType& >(payload)}
    {
        ...
        payload_.v.resize(point_.v + e2eHeaderSize);
        // Read E2E header
        std::ignore = icc_message_read_start(&point_.m, &payload_.m, 0);
        auto data{payload_.v.data() + point_.v};
        auto kSize{payload_.v.size() - point_.v};
        if (static_cast< size_t >(icc_message_read(&point_.m, data, kSize)) != kSize) {
    ...
    {
        std::ignore = icc_message_read_end(&point_.m);
        if (Message::IsRequest(payload_.m.hdr.type)
    ...
    inline ResultType Read(TValue& value, bool reverseOrder, PointType const& point) const noexcept
    {
        ...
        auto* data{reinterpret_cast< uint8_t* >(&value)};
        if (static_cast< size_t >(
                icc_message_read(const_cast< icc_message_io_t* >(&point.m), static_cast< void* >(data), kSize))
            != kSize) {
            return ErrorCode::kInvalidRead;
        }
        if (point.v + kSize > payload_.v.size()) {
