...
#ifndef __COM_ICC_SERIALIZE_H
#define __COM_ICC_SERIALIZE_H

#include <codecvt>
#include <locale>

#include "icc_serialize_stream.h"
...
{
    TimeRecorder(TimeNM::Timer, "ICCSerialize");
...
{
    TimeRecorder(TimeNM::Timer, "ICCDeserialize");
                           ...
                           std::enable_if_t< TProps::stringEncoding() == StringEncoding::kUTF8 >* = nullptr) noexcept
    {
        ...
        if (PropsType::implementsLegacyStringSerialization()) {
            if (length < kUTF8End.size()) {
                return ErrorCode::kInvalidValue;  // Read value (UTF-8 string) length field value is less than lower bound
            }
                           ...
                           std::enable_if_t< TProps::stringEncoding() == StringEncoding::kUTF16 >* = nullptr) noexcept
    {
        ...
        if (PropsType::implementsLegacyStringSerialization()) {
            if (length < kUTF16End.size() * 2) {
                return ErrorCode::kInvalidValue;  // Read value (UTF-16 string) length field value is less than lower bound
            }
...
/// @brief Template type -- converter -- specialization for struct (has tag field + has length field + enumerable members) type
/// @tparam TStream Payload stream type
/// @tparam TValue Value type
template < typename TStream, typename TValue >
struct Translator< TStream, TValue, std::enable_if_t< IsTLV< TValue >::value > >
{
    ...
    /// @ref [PRS_SOMEIP_00242] -- The configuration size of the length field for arrays, structs, unions, and strings must be the same
    ...
    static ResultType Read(StreamType& stream, ValueType& value) noexcept
    {
        ...
        if (!fields.empty()) {
            /// @ref [PRS_SOMEIP_00218] -- Should terminate if required field not found
            return ErrorCode::kInvalidValue;  // No required field in read value (TLV struct)
        }
        return ret;
    }
...
private:
    ...
    /// @brief Convert data information to wire type -- non-basic type (variable-length occupying 1 byte)
    /// @tparam T Value type
    /// @return Wire type
    template < typename T >
    static constexpr WireTypes Length2WireType(
    ...
    /// @brief Convert data information to wire type -- non-basic type (variable-length occupying 2 bytes)
    /// @tparam T Value type
    /// @return Wire type
    template < typename T >
    static constexpr WireTypes Length2WireType(
    ...
    /// @brief Convert data information to wire type -- non-basic type (variable-length occupying 4 bytes)
    /// @tparam T Value type
    /// @return Wire type
    template < typename T >
    static constexpr WireTypes Length2WireType(
...
struct Translator< TStream, TValue, std::enable_if_t< IsSequenceContainer< TValue >::value > >
{
                           ...
                           std::enable_if_t< std::is_same< ElementType, bool >::value >* = nullptr) noexcept
    {
        ...
        value.clear();
        while (len < length) {
#if 0  // `bool& v = value.back()` is disabled, vector<bool> implementation does not support returning bit reference
            value.emplace_back();
            retn = Translator< StreamType, ElementType >::Read(stream, value.back());
