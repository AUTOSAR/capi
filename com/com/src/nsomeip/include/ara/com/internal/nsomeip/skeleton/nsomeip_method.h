...
#ifndef __COM_NSOMEIP_SKELETON_METHOD_H
#define __COM_NSOMEIP_SKELETON_METHOD_H
...
public:
    ...
    NSomeipMethod(Owner& owner, std::function< Callback() >&& callback) noexcept
    ...
    /// @brief Destructor
    ~NSomeipMethod() noexcept { ComLogTrace("destroy bind skeleton method", GenArg(Desc::methodId())); }
    ...
    /// @brief Register method receiver -- called before service provision
    ...
    /// @brief Unregister method receiver -- called after stopping service provision
    /// @return Result object -- void/value or error
...
private:
    ...
    static int32_t OnRequest(nsi_app_t* a, void* ud, nsi_message_t* m) noexcept
    {
        TimeRecorder(TimeNM::Timer, "NSIOnRequest");
        std::ignore = a;
        ComLogDebug("on bind method request", GenArg(Desc::methodId()), GenArg(m));
        auto* thisObj{reinterpret_cast< NSomeipMethod* >(ud)};
        return thisObj->_handleRequest(std::shared_ptr< nsi_message_t >(m, nsi_message_close));
    }
    /// @brief Process call request message
    /// @param[in] request Request message
    /// @return Result -- 0: success; !0: failure;
    int32_t _handleRequest(std::shared_ptr< nsi_message_t > const& request) noexcept
    {
        TimeRecorder(TimeNM::Timer, "NSIHandleRequest(mode)");
    ...
    /// @param[in] request Request message
    /// @return Result -- 0: success; !0: failure;
    int32_t _handleRequest(nsi_message_t* request) noexcept
    {
        TimeRecorder(TimeNM::Timer, "NSIHandleRequest");
        TimeStart(TimeNM::Timer, "NSIHandleRequest(GetCaller)");
        auto callback{callback_()};
        TimeStop(TimeNM::Timer, "NSIHandleRequest(GetCaller)");
        NSOMEIP_ASSERT(!!callback, "handle bind method request exception: get caller failed", GenArg(Desc::serviceId()),
    ...
    int32_t _handleRequest(std::function< R(Args...) >&& callback, nsi_message_t* request) noexcept
    {
        TimeRecorder(TimeNM::Timer, "NSIHandleRequest(forward)");
    ...
    {
        TimeRecorder(TimeNM::Timer, "NSIHandleRequest(void)");
        ComLogTrace("handle bind method request", GenArg(Desc::methodId()), GenArg(request));
        TimeStart(TimeNM::Timer, "NSIHandleRequest::Verify&SecOC");
        int32_t ret{};
        ...
        if (rcode >= Message::kReturnCodeNotOk && rcode <= Message::kReturnCodeReservedMax4Generic) {
            ...
            ComLogWarning("handle bind method request rejected: verify return code invalid", GenArg(rcode),
                          GenArg(Desc::methodId()), GenArg(request));
            MonitorTrace(runtime::GetInstance(), trace::someip::kRecvRequest, trace::ServiceType::kSkeleton,
        ...
        TimeStop(TimeNM::Timer, "NSIHandleRequest::Verify&SecOC");
        TimeStart(TimeNM::Timer, "NSIHandleRequest::MakeArgs");
        ...
        if (e2eInfo_) {  // enable e2e
            ...
            if (ret < 0 || !payload.r) {
                ComLogWarning("handle bind method request rejected: deserialize E2E request failed", GenArg(ret),
                              GenArg(Desc::methodId()), GenArg(request));
                MonitorTrace(runtime::GetInstance(), trace::someip::kDeserialize, trace::ServiceType::kSkeleton,
                             Desc::serviceId(), request->inst, Desc::serviceIdentifier, Desc::methodId(), "methodName",
                             false, request->hdr.session, request->hdr.type,
                             static_cast< uint32_t >(ComErrc::kCommunicationStackError), trace::FormatArgs(""));
        ...
        TimeStop(TimeNM::Timer, "NSIHandleRequest::MakeArgs");
        TimeStart(TimeNM::Timer, "NSIHandleCall");
        ...
        TimeStop(TimeNM::Timer, "NSIHandleCall");
        return 0;
    }
    /// @brief Process call request message
    /// @tparam R Method return type
    ...
    int32_t _handleRequest(std::function< ara::core::Future< R >(Args...) >&& callback,
                           ...
                           std::index_sequence< kI... >) noexcept
    {
        TimeRecorder(TimeNM::Timer, "NSIHandleRequest(value)");
        ...
        // NSOMEIP_ASSERT(future.is_ready(), "invalid future");
        ComLogTrace("handle bind method request: invoke callback handle bind method call done",
    ...
    /// @return Result -- 0: success; !0: failure;
    template < typename T >
    int32_t _sendResponse(nsi_message_t* request, uint8_t rcode, ara::core::Result< T > const& result) const noexcept
    {
        TimeRecorder(TimeNM::Timer, "NSISendResponse");
        ComLogTrace("send bind method response", GenArg(Desc::methodId()), GenArg(request), GenArg(rcode),
                    GenArg(result));
        int32_t ret{};
        auto& runtime{runtime::GetInstance()};
        request->hdr.code = rcode;  // E2E serialized response payload requires rcode
        ...
        if (response == nullptr) {
            ...
            MonitorTrace(runtime::GetInstance(), trace::someip::kSerialize, trace::ServiceType::kSkeleton,
        ...
        if (ret < 0) {
            ComLogError("send bind method response error: send response failed", GenArg(ret), GenArg(Desc::methodId()),
                        GenArg(request), GenArg(rcode), GenArg(result));
            MonitorTrace(runtime::GetInstance(), trace::someip::kSendResponse, trace::ServiceType::kSkeleton,
                         Desc::serviceId(), request->inst, Desc::serviceIdentifier, request->hdr.method, "methodName",
                         false, request->hdr.session, static_cast< uint8_t >(trace::MsgType::kMethodResponse),
                         static_cast< uint32_t >(ComErrc::kCommunicationStackError), trace::FormatValue(result));
            return ret;
...
private:
    ...
    /// @brief Callback method getter -- used to indirectly obtain callback method
