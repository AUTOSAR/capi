...
#ifndef __COM_NSOMEIP_SIGNAL_H
#define __COM_NSOMEIP_SIGNAL_H

#include "ara/com/internal/log/log.h"
#include "nsomeip_serialize.h"
#include "nsomeip_types.h"

/// @brief Namespace -- internal binding layer signal
namespace ara {
namespace com {
namespace internal {
namespace nsomeip {
...
/// @brief Get PDU header identifier from description information
...
/// @brief Template type -- determine whether it is signal-based (no)
/// @tparam T Description information
/// @tparam Tagged Tag type for SFINAE
template < typename T, typename Tagged = void >
struct IsSignalBased : std::false_type
{
};
/// @brief Template type -- determine whether it is signal-based (yes)
/// @tparam Desc Description information
template < typename Desc >
...
/// @brief Determine whether it is signal-based
/// @tparam Desc Description information
/// @return bool
template < typename Desc >
...
/// @brief Template type -- determine whether it is a signal service (no)
/// @tparam T Description information
/// @tparam Tagged Tag type for SFINAE
template < typename T, typename Tagged = void >
...
/// @brief Template type -- determine whether it is a signal service (yes)
/// @tparam Desc Description information
template < typename Desc >
...
/// @brief Template type -- determine whether it is a SOME/IP service (no)
/// @tparam T Description information
/// @tparam Tagged Tag type for SFINAE
template < typename T, typename Tagged = void >
...
/// @brief Template type -- determine whether it is a SOME/IP service (yes)
/// @tparam Desc Description information
template < typename Desc >
...
/// @brief Determine whether it is a signal service
/// @tparam T Description information
/// @return bool
template < typename T >
...
/// @brief Determine whether it is a SOME/IP service
/// @tparam T Description information
/// @return bool
template < typename T >
...
/// @brief Determine whether it is a mixed service
/// @tparam T Description information
/// @return bool
template < typename T >
...
struct S2SProps
{
    ...
    /// @details Used to establish source/target PDU header identifier list and serialization implementation related to this value (specialized PDU header identifier)
    ...
    /// @details Used to establish fan-in/fan-out and trigger implementation related to this association (specialized source/target PDU header identifier pair)
...
/// @brief
namespace details {
/// @brief Template type -- determine whether it is a source S2S
/// @tparam id PDU header identifier
/// @tparam SourceIds Source PDU header identifier list type
template < HeaderId id, typename SourceIds >
struct IsS2SSource
{
    /// @brief Not a source PDU header identifier -- list is empty
    enum
    {
        kValue = false
    };
...
{
    /// @brief Determine whether it is a source PDU header identifier -- list has value
    enum
    {
        kValue = id == sourceId
    };
...
{
    /// @brief Determine whether source PDU header identifier exists -- list has multiple values
    enum
    {
        kValue = id == sourceId || IsS2SSource< id, HeaderIds< sourceIds... > >::kValue
    };
};
/// @brief Template type -- determine whether it is a target S2S
/// @tparam id PDU header identifier
/// @tparam TargetIds Target PDU header identifier list type
template < HeaderId id, typename TargetIds >
struct IsS2STarget
{
    /// @brief Not a target PDU header identifier -- list is empty
    enum
    {
        kValue = false
    };
...
{
    /// @brief Determine whether it is a target PDU header identifier -- list has value
    enum
    {
        kValue = id == targetId
    };
...
{
    /// @brief Determine whether target PDU header identifier exists -- list has multiple values
    enum
    {
...
/// @brief Determine whether it is a source S2S
/// @tparam Desc Description information
/// @return bool
template < typename Desc >
...
/// @brief Determine whether it is a target S2S
/// @tparam Desc Description information
/// @return bool
template < typename Desc >
