...
#ifndef __COM_NSOMEIP_PROXY_EVENT_H
#define __COM_NSOMEIP_PROXY_EVENT_H
...
class NSomeipEvent : public virtual ara::com::internal::proxy::EventBase< T >
{
public:
    ...
    explicit NSomeipEvent(Owner& owner, std::enable_if_t< !c >* = nullptr) noexcept : owner_{owner}
    {
    ...
    /// @brief Destructor
    ~NSomeipEvent() noexcept override { ComLogTrace("destroy bind proxy event", GenArg(Desc::eventId())); }
    /// @brief Copy constructor
    /// @param other
    NSomeipEvent(NSomeipEvent const& other) noexcept = default;
    /// @brief Move constructor
    /// @param other
    NSomeipEvent(NSomeipEvent&& other) noexcept = default;
    /// @brief Copy assignment operator
    /// @param other
    /// @return NSomeipEvent
    NSomeipEvent& operator=(NSomeipEvent const& other) noexcept = default;
    /// @brief Move assignment operator
    /// @param other
    /// @return NSomeipEvent
    NSomeipEvent& operator=(NSomeipEvent&& other) noexcept = default;
    /// @brief Subscribe to event
    /// @param[in] maxSampleCount Upper limit on number of samples to receive
    ...
    ara::core::Result< void > Subscribe(size_t maxSampleCount) noexcept override
    {
        ...
        if (subscriptionState_ != SubscriptionState::kNotSubscribed) {
            if (maxSampleCount != maxSampleCount_) {
                ...
                MonitorTrace(runtime::GetInstance(), trace::someip::kEventSubscribe, trace::ServiceType::kProxy,
        ...
        auto ret{nsi_appcb_set_message(runtime.GetAppCB(), Desc::serviceId(), owner_.instanceId_, Desc::eventId(),
                                       NSomeipEvent::OnNotification, this)};
        if (ret < 0) {
        ...
        nsi_event_info_t ev{};
        ev.reliability = NSI_RT_UNKNOWN;  // Client event reliability is determined by the server
        ev.type        = this->IsField() ? NSI_ET_FIELD : NSI_ET_EVENT;
    ...
    void Unsubscribe() noexcept override
    {
        ...
        ret = nsi_appcb_set_message(runtime.GetAppCB(), Desc::serviceId(), owner_.instanceId_, Desc::eventId(), nullptr,
                                    nullptr);
        if (ret < 0) {
        ...
        MonitorTrace(runtime::GetInstance(), trace::someip::kEventUnsubscribe, trace::ServiceType::kProxy,
                     Desc::serviceId(), owner_.instanceId_, Desc::serviceIdentifier, Desc::eventId(), "eventName",
                     this->IsField(), 0);
    }
    ...
    /// @param[in] maxNumberOfSamples Upper limit on number of samples to get
    /// @return Result object -- void/value or error
    /// @ref [SWS_CM_90406]
    /// @ref [SWS_CM_90407]
    /// @ref [SWS_CM_90408]
    ...
    {
        TimeRecorder(TimeNM::Timer, "NSINotify::GetNewSamples");
    ...
    {
        TimeRecorder(TimeNM::Timer, "NSINotify::GetNewSamplesImpl");
        ...
        while (true) {
            ...
            Sample sample;
            {
                ...
                /// [SWS_CM_00703] Condition 2. The subscribed maximum number of samples has been obtained from the underlying cache
                /// [SWS_CM_00703] Condition 3. No new data samples available in the underlying receive buffer
                if (queue_.empty()) {
                ...
                if (e2eInfo_) {  // enable e2e
                    ...
                    if (ret < 0) {
                        ...
                        MonitorTrace(runtime::GetInstance(), trace::someip::kDeserialize, trace::ServiceType::kProxy,
                                     ...
                                     static_cast< uint8_t >(trace::MsgType::kEventNotify),
                                     static_cast< uint32_t >(ComErrc::kCommunicationStackError),
                                     trace::FormatValue(""));
                        continue;
                    }
            ...
            /// [SWS_CM_00703] Step 4. Call the user-provided handler function referencing the sample data in the local cache
...
private:
    ...
    static int32_t OnNotification(nsi_app_t* a, void* ud, nsi_message_t* m) noexcept
    {
        TimeRecorder(TimeNM::Timer, "NSINotify::OnNotification");
        std::ignore = a;
        ...
        if (threadPool != nullptr) {
            threadPool->Dispatch([=] {
                auto* thisObj{reinterpret_cast< NSomeipEvent* >(ud)};
                thisObj->_handleNotification(std::shared_ptr< nsi_message_t >(m, nsi_message_close));
            });
            return 0;
        }
        auto* thisObj{reinterpret_cast< NSomeipEvent* >(ud)};
        return thisObj->_handleNotification(std::shared_ptr< nsi_message_t >(m, nsi_message_close));
    }
    /// @brief Process notification message
    /// @param[in] message Message
    /// @return Result -- 0: success; !0: failure;
    int32_t _handleNotification(std::shared_ptr< nsi_message_t > const& message) noexcept
    {
        TimeStart(TimeNM::Timer, "NSINotify::Verify&SecOC");
        ...
        if (rcode != Message::kReturnCodeOk) {
            ...
            MonitorTrace(
                runtime::GetInstance(), trace::someip::kRecvEvent, trace::ServiceType::kProxy, Desc::serviceId(),
...
#else
#endif
        MonitorTrace(runtime::GetInstance(), trace::someip::kRecvEvent, trace::ServiceType::kProxy, Desc::serviceId(),
                     message->inst, Desc::serviceIdentifier, Desc::eventId(), "eventName", false, message->hdr.session,
                     (ret != Message::kReturnCodeOk) ? static_cast< uint32_t >(ComErrc::kCommunicationStackError) : 0U,
                     trace::FormatValue(""));
        TimeStop(TimeNM::Timer, "NSINotify::Verify&SecOC");
        TimeRecorder(TimeNM::Timer, "NSINotify::HandleRecv");
...
#endif  // HAS_COM_EVENT_LOST_CHECKER
        {
            ...
            if (Desc::sessionHandling() == serialize::SessionHandling::kSessionHandlingActive) {
                ...
                if (sessionId == 0) {
                    // Session ID is invalid, warning + receive + do not update session
    ...
    void _setSubscriptionState(SubscriptionState state) noexcept
    {
        ...
        // TODO(jiawei-l2)
        // If the service becomes unavailable, need to change subscription status to pending and automatically renew when service becomes available (confirm whether the implementation layer already implements automatic renewal)
    ...
    template < bool c = signal::isSignalBased< Desc >() >
    int32_t _deserialize(nsi_message_t const& message, SampleType& value, std::enable_if_t< !c >* = nullptr) noexcept
    {
...
private:
    ...
    /// @brief Upper limit on number of samples to receive
    size_t maxSampleCount_{1};
    /// @brief Type alias -- receive notification queue
    using Queue = std::deque< std::shared_ptr< nsi_message_t > >;
