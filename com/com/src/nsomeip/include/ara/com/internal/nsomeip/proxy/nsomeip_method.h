...
#ifndef __COM_NSOMEIP_PROXY_METHOD_H
#define __COM_NSOMEIP_PROXY_METHOD_H

#include "../nsomeip_runtime.h"
...
using NSomeipFMethod = NSomeipMethod<
    Owner,
    Desc,
    std::enable_if_t<
...
/// @tparam T Method signature type
template < typename Owner, typename Desc, typename T >
using NSomeipVMethod = NSomeipMethod<
    Owner,
    Desc,
    std::enable_if_t<
...
/// @tparam T Method signature type
template < typename Owner, typename Desc, typename T >
using NSomeipRMethod = NSomeipMethod<
    Owner,
    Desc,
    std::enable_if_t<
...
class NSomeipMethod< Owner, Desc, void(Args...), ara::com::internal::proxy::MethodMode::kNonReturn >
...
public:
    ...
    explicit NSomeipMethod(Owner& owner) noexcept : owner_{owner}
    {
    ...
    /// @brief Destructor
    ~NSomeipMethod() noexcept override { ComLogTrace("destroy bind proxy method", GenArg(Desc::methodId())); }
    /// @brief Copy constructor
    /// @param other
    NSomeipMethod(NSomeipMethod const& other) noexcept = default;
    /// @brief Move constructor
    /// @param other
    NSomeipMethod(NSomeipMethod&& other) noexcept = default;
    /// @brief Copy assignment operator
    /// @param other
    /// @return NSomeipMethod
    NSomeipMethod& operator=(NSomeipMethod const& other) noexcept = default;
    /// @brief Move assignment operator
    /// @param other
    /// @return NSomeipMethod
    NSomeipMethod& operator=(NSomeipMethod&& other) noexcept = default;
    /// @brief Operator -- call
...
private:
    ...
    {
        TimeRecorder(TimeNM::Timer, "NSIRequest(F)");
        ...
        if (request == nullptr) {
            ComLogError("call bind method error: create request failed", GenArg(Desc::methodId()), GenArg(sessionId),
                        GenArg(args...));
            MonitorTrace(runtime::GetInstance(), trace::someip::kSerialize, trace::ServiceType::kProxy,
                         Desc::serviceId(), owner_.instanceId_, Desc::serviceIdentifier, Desc::methodId(), "methodName",
                         false, sessionId, static_cast< uint8_t >(trace::MsgType::kMethodRequestNoReturn),
                         static_cast< uint32_t >(ComErrc::kCommunicationStackError), trace::FormatArgs(args...));
            return;
        }
        MonitorTrace(runtime::GetInstance(), trace::someip::kSerialize, trace::ServiceType::kProxy, Desc::serviceId(),
                     owner_.instanceId_, Desc::serviceIdentifier, Desc::methodId(), "methodName", false, sessionId,
                     static_cast< uint8_t >(trace::MsgType::kMethodRequestNoReturn), 0U, trace::FormatArgs(args...));
        ...
        if (ret < 0) {
            ...
            MonitorTrace(runtime::GetInstance(), trace::someip::kSendRequest, trace::ServiceType::kProxy,
                         Desc::serviceId(), owner_.instanceId_, Desc::serviceIdentifier, Desc::methodId(), "methodName",
                         false, sessionId, static_cast< uint8_t >(trace::MsgType::kMethodRequestNoReturn),
                         static_cast< uint32_t >(ComErrc::kCommunicationStackError), trace::FormatArgs(args...));
            return;
        ...
        MonitorTrace(runtime::GetInstance(), trace::someip::kSendRequest, trace::ServiceType::kProxy, Desc::serviceId(),
                     owner_.instanceId_, Desc::serviceIdentifier, Desc::methodId(), "methodName", false, sessionId,
                     static_cast< uint8_t >(trace::MsgType::kMethodRequestNoReturn), 0U, trace::FormatArgs(args...));
    }
...
class NSomeipMethod< Owner, Desc, R(Args...), ara::com::internal::proxy::MethodMode::kHasReturn >
...
public:
    ...
    explicit NSomeipMethod(Owner& owner) noexcept : owner_{owner}
    {
        ...
        auto& runtime{runtime::GetInstance()};
        auto ret{nsi_appcb_set_message(runtime.GetAppCB(), Desc::serviceId(), owner_.instanceId_, Desc::methodId(),
                                       NSomeipMethod::OnResponse, this)};
        if (ret < 0) {
    ...
    /// @brief Destructor
    ~NSomeipMethod() noexcept override
    {
    ...
    /// @return Result of the last E2E_check function called
...
private:
    ...
    {
        TimeRecorder(TimeNM::Timer, "NSIRequest(R)");
        ComLogTrace("call bind method", GenArg(Desc::methodId()), GenArg(args...));
        ...
        if (request == nullptr) {
            ...
            MonitorTrace(runtime::GetInstance(), trace::someip::kSerialize, trace::ServiceType::kProxy,
                         Desc::serviceId(), owner_.instanceId_, Desc::serviceIdentifier, Desc::methodId(), "methodName",
                         false, sessionId, static_cast< uint8_t >(trace::MsgType::kMethodRequest),
                         static_cast< uint32_t >(ComErrc::kCommunicationStackError), trace::FormatArgs(args...));
        ...
        MonitorTrace(runtime::GetInstance(), trace::someip::kSerialize, trace::ServiceType::kProxy, Desc::serviceId(),
                     owner_.instanceId_, Desc::serviceIdentifier, Desc::methodId(), "methodName", false, sessionId,
                     static_cast< uint8_t >(trace::MsgType::kMethodRequest), 0U, trace::FormatArgs(args...));
        // TODO(jiawei) ara::core::Future can Cancel the method call [SWS_CM_00194]{DRAFT}
        // So timeout is handled by application layer (SWS_CM_00192)
        ...
        if (ret < 0) {
            ...
            MonitorTrace(runtime::GetInstance(), trace::someip::kSendRequest, trace::ServiceType::kProxy,
                         Desc::serviceId(), owner_.instanceId_, Desc::serviceIdentifier, Desc::methodId(), "methodName",
                         false, sessionId, static_cast< uint8_t >(trace::MsgType::kMethodRequest),
                         static_cast< uint32_t >(ComErrc::kNetworkBindingFailure), trace::FormatArgs(args...));
            {
                std::lock_guard< std::mutex > lock(mutex_);
    ...
    /// @brief Callback function when a call response message is received
    /// @param[in] a Application layer application
    /// @param[in] ud User data
    /// @param[in] m Message
    /// @return Result -- 0: success; !0: failure;
    static int32_t OnResponse(nsi_app_t* a, void* ud, nsi_message_t* m) noexcept
    {
        std::ignore = a;
        TimeRecorder(TimeNM::Timer, "NSIOnResponse");
        ComLogDebug("on bind method response", GenArg(Desc::methodId()), GenArg(m));
        MonitorTrace(runtime::GetInstance(), trace::someip::kRecvResponse, trace::ServiceType::kProxy,
                     Desc::serviceId(), m->inst, Desc::serviceIdentifier, Desc::methodId(), "methodName", false,
                     m->hdr.session, static_cast< uint8_t >(trace::MsgType::kMethodResponse), 0U,
                     trace::FormatArgs(""));
        ...
        if (threadPool != nullptr) {
            threadPool->Dispatch([=] {
                auto* thisObj{reinterpret_cast< NSomeipMethod* >(ud)};
                std::ignore = thisObj->_handleResponse(std::shared_ptr< nsi_message_t >(m, nsi_message_close));
            });
            return 0;
        }
        auto* thisObj{reinterpret_cast< NSomeipMethod* >(ud)};
        return thisObj->_handleResponse(std::shared_ptr< nsi_message_t >(m, nsi_message_close));
    }
    /// @brief Process call response message
    /// @param[in] response Response message
    /// @return Result -- 0: success; !0: failure;
    /// @ref [SWS_CM_10358]
    int32_t _handleResponse(std::shared_ptr< nsi_message_t > const& response) noexcept
    {
        ...
        TimeStart(TimeNM::Timer, "NSIResponse::Verify&SecOC");
...
#else
#endif
        TimeStop(TimeNM::Timer, "NSIResponse::Verify&SecOC");
        ...
        if (Message::IsResponse(response->hdr.type) && response->hdr.code > Message::kReturnCodeReservedMax4Generic) {
            ...
            if (ret >= 0) {
                ...
                MonitorTrace(runtime::GetInstance(), trace::someip::kDeserialize, trace::ServiceType::kProxy,
                             Desc::serviceId(), response->inst, Desc::serviceIdentifier, Desc::methodId(), "methodName",
                             false, response->hdr.session, static_cast< uint8_t >(trace::MsgType::kMethodResponse), 0U,
                             trace::FormatValue(value));
        ...
        if (response->hdr.code != Message::kReturnCodeOk) {
            ...
            if (e2eInfo_) {  // enable e2e
                ...
                if (ret < 0) {
                    ...
                    MonitorTrace(runtime::GetInstance(), trace::someip::kDeserialize, trace::ServiceType::kProxy,
                                 Desc::serviceId(), response->inst, Desc::serviceIdentifier, Desc::methodId(),
                                 "methodName", false, response->hdr.session,
                                 static_cast< uint8_t >(trace::MsgType::kMethodResponse),
    ...
    /// @brief Process call response message -- return value
    /// @tparam U Return type
    /// @param[in] response Response message
    /// @param[in] promise Promise of the related call request
    /// @return Result -- 0: success; !0: failure;
    template < typename U = Output >
    int32_t _handleResponse(std::shared_ptr< nsi_message_t > const& response,
                            ara::core::Promise< Output >& promise,
                            std::enable_if_t< !std::is_same< U, void >::value >* = nullptr) noexcept
    {
        TimeRecorder(TimeNM::Timer, "NSIHandleResponse(value)");
        Output value;
        int32_t ret{};
        if (e2eInfo_) {  // enable e2e
            serialize::E2EPayload payload{*response, e2eInfo_->dataId};
            ...
            if (ret < 0) {
                ...
                MonitorTrace(runtime::GetInstance(), trace::someip::kDeserialize, trace::ServiceType::kProxy,
                             Desc::serviceId(), response->inst, Desc::serviceIdentifier, Desc::methodId(), "methodName",
                             false, response->hdr.session, static_cast< uint8_t >(trace::MsgType::kMethodResponse),
                             static_cast< uint32_t >(ComErrc::kCommunicationStackError), trace::FormatArgs(""));
                promise.SetError(MakeErrorCode(ComErrc::kCommunicationStackError, ret));
                return ret;
            }
        ...
        ComLogDebug("handle bind method response: call bind method done", GenArg(value), GenArg(Desc::methodId()),
                    GenArg(response));
        MonitorTrace(runtime::GetInstance(), trace::someip::kDeserialize, trace::ServiceType::kProxy, Desc::serviceId(),
                     response->inst, Desc::serviceIdentifier, Desc::methodId(), "methodName", false,
                     response->hdr.session, static_cast< uint8_t >(trace::MsgType::kMethodResponse), 0U,
                     trace::FormatValue(value));
        promise.set_value(std::move(value));
        return 0;
    }
    /// @brief Process call response message -- void return
    /// @tparam U Return type
    /// @param[in] response Response message
    /// @param[in] promise Promise of the related call request
    /// @return Result -- 0: success; !0: failure;
    ...
    int32_t _handleResponse(std::shared_ptr< nsi_message_t > const& response,
                            ...
                            std::enable_if_t< std::is_same< U, void >::value >* = nullptr) noexcept
    {
        TimeRecorder(TimeNM::Timer, "NSIHandleResponse(void)");
        int32_t ret{};
        if (e2eInfo_) {  // enable e2e
            serialize::E2EPayload payload{*response, e2eInfo_->dataId};
