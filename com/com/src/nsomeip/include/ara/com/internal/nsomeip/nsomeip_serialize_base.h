...
enum ErrorCode
{
    ...
    kInvalidPoint = -4,  ///< Invalid point
    kInvalidValue = -5   ///< Invalid value
};
/// @brief Byte order mode enumeration
enum class ByteOrder : uint8_t
{
    kMostSignificantByteFirst,  ///< High byte first (big-endian)
    kMostSignificantByteLast,   ///< Low byte first (little-endian)
...
/// @brief Alignment bit occupancy enumeration
...
/// @brief Length field byte occupancy enumeration
...
{
    /// @brief Get alignment to bit @ref [TPS_MANI_03073]
    ...
    /// @brief Get sequence/associative map container length field placeholder (extended fixed-length array) @ref [TPS_MANI_03070]
    /// @details shall be either 0, 1, 2 or 4 @ref [constr_3353]
    /// @details 0 is only allowed to be used if a fixed size array @ref [constr_3447]
    /// @details in case of TLV, sizeof*LengthField shall have an identical value and greater than 0 @ref [constr_1628]
    /// @return uint8_t -- default 4 @ref [SWS_CM_00258] [SWS_CM_10258]
...
/// @brief Template type -- determine whether it is a basic (scalar) type
/// @tparam T Value type
template < typename T >
using IsScalar = std::is_scalar< T >;
/// @brief Template type -- determine whether it is a string type (no)
/// @tparam T Value type
template < typename T >
struct IsString : std::false_type
{
};
/// @brief Template type -- determine whether it is a string type (yes)
template <>
struct IsString< ara::core::String > : std::true_type
{
};
/// @brief Template type -- determine whether it is a structure (parameter list + enumerable members) type (no)
/// @tparam T Value type
...
/// @brief Template type -- determine whether it is a structure (parameter list + enumerable members) type (yes)
template < typename T >
...
/// @brief Template type -- determine whether it is a structure (enumerable members) type (no)
/// @tparam T Value type
/// @tparam Tagged A tag to enable SFINAE.
template < typename T, typename Tagged = void >
...
/// @brief Template type -- determine whether it is a structure (enumerable members) type (yes)
template < typename T >
...
/// @brief Template type -- determine whether it is a structure (with length field + enumerable members) type (yes)
template < typename T >
...
/// @brief Template type -- determine whether it is a structure (with tag field + length field + enumerable members) type (no)
/// @tparam T Value type
/// @tparam Tagged A tag to enable SFINAE.
template < typename T, typename Tagged = void >
...
/// @brief Template type -- determine whether it is a structure (with tag field + length field + enumerable members) type (yes)
template < typename T >
...
/// @brief Template type -- determine whether it is a structure (S2S conversion) type (no)
/// @tparam T Value type
/// @tparam Tagged A tag to enable SFINAE.
template < typename T, typename Tagged = void >
...
/// @brief Template type -- determine whether it is a structure (S2S conversion) type (yes)
template < typename T >
...
/// @brief Template type -- determine whether it is a container type (no)
/// @tparam T Value type
...
/// @brief Template type -- determine whether it is a container type (yes)
template < typename... T, template < typename... > class C >
struct IsContainer< C< T... >, C > : std::true_type
{
};
/// @brief Template type -- determine whether it is a sequence container type
/// @tparam T Value type
template < typename T >
using IsSequenceContainer
    = std::conditional_t< IsContainer< T, ara::core::Vector >::value || IsContainer< T, std::vector >::value,
                          std::true_type,
                          std::false_type >;
/// @brief Template type -- determine whether it is an associative map container type
/// @tparam T Value type
template < typename T >
using IsAssociativeContainer
    = std::conditional_t< IsContainer< T, ara::core::Map >::value || IsContainer< T, std::map >::value
                          ...
                          std::true_type,
                          std::false_type >;
/// @brief Template type -- determine whether extended fixed-length array length field is supported (no)
/// @tparam T
template < typename T, typename = void >
struct EnableFixedArrayLengthField : std::false_type
{
};
/// @brief Template type -- determine whether extended fixed-length array length field is supported (yes)
/// @tparam T
template < typename T >
...
/// @brief Template type -- determine whether extended fixed-length string length field is supported (no)
/// @tparam T
template < typename T, typename = void >
struct EnableFixedStringLengthField : std::false_type
{
};
/// @brief Template type -- determine whether extended fixed-length string length field is supported (yes)
/// @tparam T
template < typename T >
...
public:
    ...
    /// @brief Get pointer to data start position
    /// @return Pointer to data start position
    constexpr ValueType* Begin() const noexcept { return data_; }
    /// @brief Get pointer to data end position
    /// @return Pointer to data end position
    constexpr ValueType* End() const noexcept { return data_ + size_; }
    /// @brief Check whether data is empty
    /// @return bool
