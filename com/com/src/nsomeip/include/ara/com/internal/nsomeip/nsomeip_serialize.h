...
#ifndef __COM_NSOMEIP_SERIALIZE_H
#define __COM_NSOMEIP_SERIALIZE_H

#include <codecvt>
#include <locale>

#include "nsomeip_serialize_stream.h"
...
namespace nsomeip {
namespace serialize {
/// @brief Serialize value list to payload (void) and return number of bytes written or error code
/// @tparam TProps Property information
/// @tparam TValues Value list type
/// @param[in] payload Payload (void)
...
{
    TimeRecorder(TimeNM::Timer, "NSISerialize");
...
{
    TimeRecorder(TimeNM::Timer, "NSIDeserialize");
...
/// @brief Template type -- converter -- specialization for scalar (basic) type
/// @tparam TStream Payload stream type
/// @tparam TValue Value type
template < typename TStream, typename TValue >
...
struct Translator< TStream, TValue, std::enable_if_t< IsString< TValue >::value > >
{
                           ...
                           std::enable_if_t< TProps::stringEncoding() == StringEncoding::kUTF8 >* = nullptr) noexcept
    {
        ...
        if (PropsType::sizeofStringLengthField() != 0) {
            try {
                ...
                return ErrorCode::kInvalidValue;  // Read value (UTF-8 string) length field value assignment failed
            }
            if (!value.empty()) {
                           ...
                           std::enable_if_t< TProps::stringEncoding() == StringEncoding::kUTF16 >* = nullptr) noexcept
    {
        ...
        std::u16string u16;
        if (PropsType::sizeofStringLengthField() != 0) {
            while (len < (length - 1)) {  // Not termination (including odd)
                uint16_t c{};
                retn = stream.Read(c, reverseOrder);
                if (retn < 0) {
                    return retn;
            ...
            if (len == (length - 1)) {  // Ignore value (UTF-16 string), compatible with odd termination
                uint8_t c{};
                retn = stream.Read(c, reverseOrder);
                if (retn < 0) {
                    return retn;
...
struct Translator< TStream, TValue, std::enable_if_t< IsTLV< TValue >::value > >
{
    ...
    /// @ref [PRS_SOMEIP_00242] -- The configured size of length fields for arrays, structures, unions, and strings must be the same
    ...
    static ResultType Read(StreamType& stream, ValueType& value) noexcept
    {
        ...
        if (!fields.empty()) {
            ...
            return ErrorCode::kInvalidValue;  // Required field not present in read value (TLV structure)
        }
        return ret;
    }
...
private:
    ...
    struct Writer
    {
        ...
        /// @ref [PRS_SOMEIP_00212] -- Basic data types (wire type 0-3) configured with Data ID do not insert length field
        /// @ref [PRS_SOMEIP_00214] -- Non-basic data types (wire type 4-7) configured with Data ID insert length field (once)
        template < typename T >
    ...
    struct Reader
    {
        ...
        void operator()(T& value) noexcept
        {
            ...
            if (typeLength == 0) {
                // No members remaining in read value (TLV structure)
                return;
            }
            ResultType retn{};
    ...
    /// @brief Convert data information to tag
    /// @tparam T Value type
    /// @param[in] tag Tag
    /// @param[in] field Field identifier
    template < typename T >
    ...
    /// @brief Convert tag to data information
    /// @param[in] tag Tag
    /// @param[in] field Field identifier
    ...
    /// @brief Convert data information to wire type -- 1-byte basic type
    /// @tparam T Value type
    /// @return Wire type
    template < typename T >
    ...
    /// @brief Convert data information to wire type -- 2-byte basic type
    /// @tparam T Value type
    /// @return Wire type
    template < typename T >
    static constexpr WireTypes Length2WireType(
    ...
    /// @brief Convert data information to wire type -- 4-byte basic type
    /// @tparam T Value type
    /// @return Wire type
    template < typename T >
    static constexpr WireTypes Length2WireType(
    ...
    /// @brief Convert data information to wire type -- 8-byte basic type
    /// @tparam T Value type
    /// @return Wire type
    template < typename T >
    static constexpr WireTypes Length2WireType(
    ...
    /// @brief Convert data information to wire type -- non-basic type (fixed-length)
    /// @tparam T Value type
    /// @return Wire type
    template < typename T >
    static constexpr WireTypes Length2WireType(
    ...
    /// @brief Convert data information to wire type -- non-basic type (variable-length, 1 byte)
    /// @tparam T Value type
    /// @return Wire type
    template < typename T >
    static constexpr WireTypes Length2WireType(
    ...
    /// @brief Convert data information to wire type -- non-basic type (variable-length, 2 bytes)
    /// @tparam T Value type
    /// @return Wire type
    template < typename T >
    static constexpr WireTypes Length2WireType(
    ...
    /// @brief Convert data information to wire type -- non-basic type (variable-length, 4 bytes)
    /// @tparam T Value type
    /// @return Wire type
    template < typename T >
    static constexpr WireTypes Length2WireType(
    ...
    /// @brief Convert data information to wire type -- optional value type
    /// @tparam T Value type
    /// @return Wire type
    template < typename T >
    static constexpr WireTypes Length2WireType(
...
struct Translator< TStream, TValue, std::enable_if_t< IsSequenceContainer< TValue >::value > >
{
                           ...
                           std::enable_if_t< !std::is_same< ElementType, bool >::value >* = nullptr) noexcept
    {
        ...
        if (!IsScalar< ElementType >::value || sizeof(ElementType) != 1) {
            ...
            try {
                ...
                return ErrorCode::kInvalidValue;  // Read value (dynamic array) length field value assignment failed
            }
            StringBorrow cstring(reinterpret_cast< char* >(value.data()), value.size());
            retn = stream.Read(cstring);
            if (retn < 0) {
                           ...
                           std::enable_if_t< std::is_same< ElementType, bool >::value >* = nullptr) noexcept
    {
        ...
        value.clear();
        while (len < length) {
#if 0  // Disabled `bool& v = value.back()`, vector<bool> implementation does not support returning bit reference
            value.emplace_back();
            retn = Translator< StreamType, ElementType >::Read(stream, value.back());
