...
#ifndef __COM_NSOMEIP_SKELETON_EVENT_H
#define __COM_NSOMEIP_SKELETON_EVENT_H
...
class NSomeipEvent : public virtual ara::com::internal::skeleton::EventBase< T >
{
public:
    ...
    /// @brief Destructor
    ~NSomeipEvent() noexcept override { ComLogTrace("destroy bind skeleton event", GenArg(Desc::eventId())); }
    ...
    /// @brief Provide event notification -- called after service provision
    ...
    ara::core::Result< void > OfferNotification() noexcept
    {
        ...
        nsi_event_info_t ev{};
        ev.reliability = NSI_RT_UNKNOWN;  // TODO(jiawei-l5) The reliability of server-side events is currently only configured through config
        ...
        auto ret{nsi_app_offer_event(runtime.GetApp(), Desc::serviceId(), owner_.instanceId_, Desc::eventId(), &ev)};
        if (ret < 0) {
    ...
    /// @brief Stop providing event notification -- called before stopping service provision
    /// @return Result object -- void/value or error
    ara::core::Result< void > StopOfferNotification() noexcept
    {
        ...
        auto& runtime{runtime::GetInstance()};
        auto ret{nsi_app_stop_event(runtime.GetApp(), Desc::serviceId(), owner_.instanceId_, Desc::eventId())};
        if (ret < 0) {
    ...
    {
        TimeRecorder(TimeNM::Timer, "NSINotify::Send(value)");
        using Result = ara::core::Result< void >;
        ...
        if (ret < 0) {
            ...
            MonitorTrace(runtime::GetInstance(), trace::someip::kSendEvent, trace::ServiceType::kSkeleton,
                         Desc::serviceId(), owner_.instanceId_, Desc::serviceIdentifier, Desc::eventId(), "eventName",
