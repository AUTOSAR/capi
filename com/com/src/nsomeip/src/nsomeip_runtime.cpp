...
std::unique_ptr< NSomeipRuntime > g_NSomeipRuntime{};
ara::core::Result< void > Initialize(bool isDaemon) noexcept
{
    using Result = ara::core::Result< void >;
    ComLogTrace("initialize bind runtime", GenArg(isDaemon), GenArg(g_NSomeipRuntime));
    Result result{};
    assert(g_NSomeipRuntime == nullptr);
    g_IsDaemon       = isDaemon;
    g_NSomeipRuntime = std::make_unique< NSomeipRuntime >();
    if (auto res{g_NSomeipRuntime->Initialize()}) {
    } else {
        std::ignore = Deinitialize();
        result      = std::move(res);
        ComLogError("initialize bind runtime error", GenArg(result), GenArg(isDaemon), GenArg(g_NSomeipRuntime));
        return result;
    }
    ComLogDebug("initialize bind runtime done", GenArg(isDaemon), GenArg(g_NSomeipRuntime));
    return result;
}
ara::core::Result< void > Deinitialize() noexcept
{
    using Result = ara::core::Result< void >;
    ComLogTrace("deinitialize bind runtime", GenArg(g_NSomeipRuntime));
    Result result{};
    assert(g_NSomeipRuntime != nullptr);
    if (auto res{g_NSomeipRuntime->Deinitialize()}) {
    } else {
        result = std::move(res);
    }
    g_NSomeipRuntime = nullptr;
    ComLogDebug("deinitialize bind runtime done", GenArg(g_NSomeipRuntime));
    return result;
}
...
ara::core::Optional< ServiceInfo::InstanceId > ToNSomeipInstanceId(InstanceIdentifier const& input) noexcept
{
    ...
    try {
        ...
        // Value conversion failed, return empty
...
#if defined(NSI_NET_FILTER)
struct NsomeipSignalWrapper
{
    nsi_routing_t* routing;
    nai_signal_t signal;
};
#endif

/// @brief Binding layer runtime implementation type
class NSomeipRuntime::Impl : public internal::BindRuntime
{
public:
    /// @brief Constructor
    Impl() noexcept
    {
        ...
        NSOMEIP_ASSERT(nsi_conf_init(&conf_), -__LINE__);
        NSOMEIP_ASSERT(nsi_routing_init(&routing_), -__LINE__);
        NSOMEIP_ASSERT(nsi_app_init(&app_), -__LINE__);
        if (!g_IsDaemon) {
            NSOMEIP_ASSERT(nsi_appcb_init(&appcb_), -__LINE__);
        }
    }
    /// @brief Destructor
    ...
    ara::core::Result< void > Initialize() noexcept
    {
        ...
        Result result{};
        if (auto res{NSOMEIP_CHECK(InitializeBind(), -__LINE__)}) {
            deinitializes_.emplace_front([&] { return NSOMEIP_CHECK(DeinitializeBind(), -__LINE__); });
        } else {
            result = std::move(res);
            return result;
        }
        if (auto res{NSOMEIP_CHECK(InitializeNSI(), -__LINE__)}) {
            deinitializes_.emplace_front([&] { return NSOMEIP_CHECK(DeinitializeNSI(), -__LINE__); });
        } else {
            result = std::move(res);
            return result;
        }
        if (auto res{NSOMEIP_CHECK(InitializeE2E(), -__LINE__)}) {
            deinitializes_.emplace_front([&] { return NSOMEIP_CHECK(DeinitializeE2E(), -__LINE__); });
        } else {
            result = std::move(res);
            return result;
        }
#ifdef HAS_COM_SECOC
        if (auto res{NSOMEIP_CHECK(InitializeSecOC(), -__LINE__)}) {
            deinitializes_.emplace_front([&] { return NSOMEIP_CHECK(DeinitializeSecOC(), -__LINE__); });
        } else {
            result = std::move(res);
            return result;
        }
...
#endif
#ifdef HAS_ARA_IAM
        if (auto res{NSOMEIP_CHECK(InitializeIAM(), -__LINE__)}) {
            deinitializes_.emplace_front([&] { return NSOMEIP_CHECK(DeinitializeIAM(), -__LINE__); });
        } else {
            result = std::move(res);
            return result;
        }
    ...
    void RegisterProxyMapper(std::shared_ptr< ProxyMapperBase > const& mapper) noexcept
    {
        ...
        NSOMEIP_ASSERT(mapper1 == nullptr && mapper2 == nullptr, "register proxy mapper repeat");
    ...
    void RegisterSkeletonMapper(std::shared_ptr< SkeletonMapperBase > const& mapper) noexcept
    {
        ...
        NSOMEIP_ASSERT(mapper1 == nullptr && mapper2 == nullptr, "register skeleton mapper repeat");
        mapper1 = mapper2 = mapper;
    }

#if defined(NSI_NET_FILTER)
    static nai_int_t SignalCloseStub(void* ctx) noexcept
    {
        nai_signal_t* signal;
        signal = static_cast< nai_signal_t* >(ctx);
        nai_signal_close(signal);
        return 0;
    };

    static nai_int_t SignalCallback(nai_signal_t* s, nai_int_t events) noexcept
    {
        nai_int_t r;
        nai_int_t signum;
        NsomeipSignalWrapper* wrapper;
    ...
    #endif
        wrapper = static_cast< NsomeipSignalWrapper* >(nai_containof(s, NsomeipSignalWrapper, signal));
    #if defined(__clang__)
        ...
        switch (events & NAI_EV_MASK) {
            case NAI_EV_SIGNAL:
                ...
                if (signum == SIGRTMIN) {
                    // open incoming&outgoing data flow
                    ComLogDebug("receive signal to open incoming&outgoing data flow");
                    nsi_routing_set_net_filter(wrapper->routing, 1, 0);
                    nsi_routing_set_net_filter(wrapper->routing, 0, 0);
                } else if (signum == SIGRTMIN + 1) {
                    // close incoming&outgoing data flow
                    ComLogDebug("receive signal to close incoming&outgoing data flow");
                    nsi_routing_set_net_filter(wrapper->routing, 1, 1);
                    nsi_routing_set_net_filter(wrapper->routing, 0, 1);
                } else if (signum == SIGRTMIN + 2) {
                    // only open incoming data flow
                    ComLogDebug("receive signal to only open incoming data flow");
                    nsi_routing_set_net_filter(wrapper->routing, 1, 0);
                    nsi_routing_set_net_filter(wrapper->routing, 0, 1);
                } else if (signum == SIGRTMIN + 3) {
                    // only open outgoing data flow
                    ComLogDebug("receive signal to only open outgoing data flow");
                    nsi_routing_set_net_filter(wrapper->routing, 0, 0);
                    nsi_routing_set_net_filter(wrapper->routing, 1, 1);
                };
                break;
...
public:
    ...
    ara::core::Result< void > InitializeBind() noexcept
    {
        ...
        if (!siManifestRes) {
            // Ignore here, initializing the communication runtime previously maps instance descriptors and instance identifier sets; if opening fails here, it only means the application does not require instance deployment configuration
            ...
            if (key3 == "name") {
                ...
                std::ignore = node3.IterateObject("", [&](auto& key4, auto& node4) {
                    ...
                    if (key4 == "initial_offer_behavior") {
                        ...
                        // [TODO] Check impact on platform startup, uncomment after upgrade implementer is ready
    ...
    /// @return Result object -- void/value or error
    ara::core::Result< void > InitializeNSI() noexcept
    {
        using Result = ara::core::Result< void >;
        ...
        isoft::ara_fsh::Process proc;
        auto manifestFile{proc.GetConfig("nsomeip.json")};
        // constexpr auto manifestFile{"etc/nsomeip.json"};
        ...
        if (!manifestRes) {
            // Ignore if there is no service application
        ...
        ret = NSOMEIP_CHECK(nsi_conf_finish(&conf_), -__LINE__);
        if (ret < 0) {
            result.EmplaceError(MakeErrorCode(ComErrc::kCommunicationStackError, -__LINE__));
            return result;
        }
        ret = NSOMEIP_CHECK(nsi_routing_set_conf(&routing_, &conf_), -__LINE__);
        if (ret < 0) {
            result.EmplaceError(MakeErrorCode(ComErrc::kCommunicationStackError, -__LINE__));
            return result;
        }
        // Open implementation layer routing
        ret = NSOMEIP_CHECK(nsi_routing_open(&routing_, evloop_->GetRawEvLoop(), NSI_RTYPE_AUTO), -__LINE__);
        if (ret < 0) {
            result.EmplaceError(MakeErrorCode(ComErrc::kCommunicationStackError, -__LINE__));
            return result;
        }
#if defined(NSI_NET_FILTER)
        {
            nai_signal_init(&allOpen.signal);
            nai_signal_init(&allClose.signal);
            nai_signal_init(&onlyIncomingOpen.signal);
            nai_signal_init(&onlyOutgoingOpen.signal);
            allOpen.routing          = &routing_;
            allClose.routing         = &routing_;
            onlyIncomingOpen.routing = &routing_;
            onlyOutgoingOpen.routing = &routing_;
            nai_signal_set_cb(&allOpen.signal, SignalCallback);
            nai_signal_set_cb(&allClose.signal, SignalCallback);
            nai_signal_set_cb(&onlyIncomingOpen.signal, SignalCallback);
            nai_signal_set_cb(&onlyOutgoingOpen.signal, SignalCallback);
            ComLogTrace("set signals for nsomeip net filter");
            if (nai_signal_open(&allOpen.signal, evloop_->GetRawEvLoop(), SIGRTMIN) < 0) {
                ComLogError("set SIGRTMIN signal to open incoming&outgoing data flow error");
                result.EmplaceError(MakeErrorCode(ComErrc::kCommunicationStackError, -__LINE__));
                return result;
            };
            if (nai_signal_open(&allClose.signal, evloop_->GetRawEvLoop(), SIGRTMIN + 1) < 0) {
                ComLogError("set SIGRTMIN+1 signal to close incoming&outgoing data flow error");
                result.EmplaceError(MakeErrorCode(ComErrc::kCommunicationStackError, -__LINE__));
                return result;
            };
            if (nai_signal_open(&onlyIncomingOpen.signal, evloop_->GetRawEvLoop(), SIGRTMIN + 2) < 0) {
                ComLogError("set SIGRTMIN+2 signal to only open incoming data flow error");
                result.EmplaceError(MakeErrorCode(ComErrc::kCommunicationStackError, -__LINE__));
                return result;
            };
            if (nai_signal_open(&onlyOutgoingOpen.signal, evloop_->GetRawEvLoop(), SIGRTMIN + 3) < 0) {
                ComLogError("set SIGRTMIN+3 signal to only open outgoing data flow error");
                result.EmplaceError(MakeErrorCode(ComErrc::kCommunicationStackError, -__LINE__));
                return result;
            };
        };
        ...
        auto& comRuntime{internal::GetInstance()};
        ret = NSOMEIP_CHECK(nsi_app_open_name(&app_, &routing_, comRuntime.GetProcessName().data()), -__LINE__);
        if (ret < 0) {
            result.EmplaceError(MakeErrorCode(ComErrc::kCommunicationStackError, -__LINE__));
            return result;
        }
        if (!g_IsDaemon) {
            ret = NSOMEIP_CHECK(nsi_appcb_open(&appcb_, &app_), -__LINE__);
            if (ret < 0) {
                result.EmplaceError(MakeErrorCode(ComErrc::kCommunicationStackError, -__LINE__));
                return result;
            }
    ...
    /// @return Result object -- void/value or error
    ara::core::Result< void > DeinitializeNSI() noexcept
    {
        using Result = ara::core::Result< void >;
...
#if defined(NSI_NET_FILTER)
        {
            if (nai_evloop_in_dispatch(evloop_->GetRawEvLoop()) == 0) {
                nai_evloop_exec(evloop_->GetRawEvLoop(), SignalCloseStub, &allOpen.signal);
                nai_evloop_exec(evloop_->GetRawEvLoop(), SignalCloseStub, &allClose.signal);
                nai_evloop_exec(evloop_->GetRawEvLoop(), SignalCloseStub, &onlyIncomingOpen.signal);
                nai_evloop_exec(evloop_->GetRawEvLoop(), SignalCloseStub, &onlyOutgoingOpen.signal);
            } else {
                nai_signal_close(&allOpen.signal);
                nai_signal_close(&allClose.signal);
                nai_signal_close(&onlyIncomingOpen.signal);
                nai_signal_close(&onlyOutgoingOpen.signal);
            };
        }
#endif
        int32_t ret{};
        if (!g_IsDaemon) {
            ret = NSOMEIP_CHECK(nsi_appcb_close(&appcb_), -__LINE__);
            if (ret < 0) {
                result.EmplaceError(MakeErrorCode(ComErrc::kCommunicationStackError, -__LINE__));
            }
        }
        // Close implementation layer application
        ret = NSOMEIP_CHECK(nsi_app_close(&app_), -__LINE__);
        if (ret < 0) {
            result.EmplaceError(MakeErrorCode(ComErrc::kCommunicationStackError, -__LINE__));
        }
        // Close implementation layer routing
        ret = NSOMEIP_CHECK(nsi_routing_close(&routing_), -__LINE__);
        if (ret < 0) {
            result.EmplaceError(MakeErrorCode(ComErrc::kCommunicationStackError, -__LINE__));
        }
        // Unload implementation layer configuration
        ret = NSOMEIP_CHECK(nsi_conf_close(&conf_), -__LINE__);
        if (ret < 0) {
            result.EmplaceError(MakeErrorCode(ComErrc::kCommunicationStackError, -__LINE__));
        }
        // Unbind event loop
    ...
    ara::core::Result< void > InitializeIAM() noexcept
    {
        ...
        if (!g_IsDaemon) {
            ...
            if (auto res{ara::iam::internal::grant::IAMGrantComQuery::Initialize()}) {
                nsi_app_set_cb_offer(&app_, Impl::OnIAMOfferService);
                nsi_app_set_cb_find(&app_, Impl::OnIAMFindService);
                nsi_app_set_cb_subscribe(&app_, Impl::OnIAMSubscribe);
                nsi_app_set_cb_message(&app_, Impl::OnIAMMessage);
    ...
    ara::core::Result< void > DeinitializeIAM() noexcept
    {
        ...
        if (!g_IsDaemon) {
            ...
            nsi_app_set_cb_offer(&app_, nullptr);
            nsi_app_set_cb_find(&app_, nullptr);
            nsi_app_set_cb_subscribe(&app_, nullptr);
            nsi_app_set_cb_message(&app_, nullptr);
...
private:
    ...
    /// @brief Type alias -- discovery service handle to callback data multi-map
    using FindHandle2CBData
        = std::multimap< FindServiceHandle, std::shared_ptr< FindHandleCBData >, FindHandleComparer >;
    /// @brief Discovery service handle to callback data multi-map
    ...
    struct GroupInfo
    {
        ...
        ara::core::Map< nsi_gid_t, ara::core::Vector< nsi_eid_t > > gid2Ids;
        /// @brief Event identifier to subscription event confirmation callback mapping
        ara::core::Map< nsi_eid_t, std::function< void(int32_t rcode) > > ackId2Listener;
    };
...
#if defined(NSI_NET_FILTER)
    NsomeipSignalWrapper allClose;
    NsomeipSignalWrapper allOpen;
    NsomeipSignalWrapper onlyIncomingOpen;
    NsomeipSignalWrapper onlyOutgoingOpen;
#endif
...
public:
    ...
    int32_t OfferService(ServiceInfo::InstanceUID const& instanceUID) noexcept
    {
        ...
        auto ret{NSOMEIP_CHECK(nsi_app_offer_service(&app_, std::get< 0 >(instanceUID), std::get< 1 >(instanceUID),
        ...
        ret = NSOMEIP_CHECK(
            nsi_appcb_set_subscribe(&appcb_, std::get< 0 >(instanceUID), std::get< 1 >(instanceUID), Impl::OnSubscribe),
    ...
    int32_t StopService(ServiceInfo::InstanceUID const& instanceUID) noexcept
    {
        ...
        int32_t result{};
        auto ret{NSOMEIP_CHECK(
            nsi_appcb_set_subscribe(&appcb_, std::get< 0 >(instanceUID), std::get< 1 >(instanceUID), nullptr),
            "stop stack service error: invoke stack unset subscribe callback failed", GenArg(instanceUID))};
        if (ret < 0) {
            result = ret;
        }
        ret = NSOMEIP_CHECK(nsi_app_stop_service(&app_, std::get< 0 >(instanceUID), std::get< 1 >(instanceUID),
                                                 std::get< 2 >(instanceUID), std::get< 3 >(instanceUID)),
...
private:
#ifdef HAS_ARA_IAM
    /// @brief Receive service provision
    /// @param[in] a Application layer application
    ...
    /// @return Result -- 0: success; !0: failure;
    static int32_t OnIAMOfferService(nsi_app_t* a,
                                     void* ud,
                                     nsi_serv_t serv,
                                     nsi_inst_t inst,
                                     nsi_version_t major,
                                     nsi_versmin_t minor,
                                     nsi_cred_t const* cred) noexcept
    {
        std::ignore = a;
        std::ignore = ud;
        std::ignore = major;
...
public:
                                        ...
                                        bindHandles) noexcept override
    {
        ...
        for (auto it{rMappers.begin()}; it != rMappers.end(); ++it) {
            ...
            // Trigger a query, the callback destructor will automatically end the query
            ...
            nai_array_t out;
            auto ret{NSOMEIP_CHECK(nai_array_init(&out, sizeof(nsi_service_info_t), nullptr), -__LINE__)};
            if (ret < 0) {
                continue;
            }
            ret = NSOMEIP_CHECK(nsi_appcb_get_availables(&appcb_, serviceId, instanceId, &out), GenArg(serviceId),
                                GenArg(instanceId));
            if (ret < 0) {
                std::ignore = nai_array_close(&out);
                continue;
            }
            auto* availableInfo{reinterpret_cast< nsi_service_info_t* >(out.elts)};
            ...
            std::ignore = nai_array_close(&out);
            {
                ...
                for (auto itAvailableInstanceInfo{availableInstanceInfos_.begin()};
                    ...
                    auto available{
                        nsi_appcb_is_available(&appcb_, std::get< 0 >(instanceUID), std::get< 1 >(instanceUID))};
                    auto instanceAvailable{available == NSI_AVAIL_CHANGED || available == NSI_AVAIL_ONLINED};
                                       ...
                                       findHandler) noexcept override
    {
        ...
        auto optionalId{runtime::ToNSomeipInstanceId(findHandle.instanceIdentifier)};
        if (!optionalId) {  // Conversion failed, instance identifier not supported
        ...
        for (auto it{rMappers.begin()}; it != rMappers.end(); ++it) {
            ...
            // Synchronize this layer's callback data before registration (since the callback process involves both this layer and underlying double locks, to ensure concurrency, avoid deadlock, and maintain registration query order, use smart pointer solution: reserve first, then construct to trigger query)
            ...
            if (enableSetAvailable) {
                NSOMEIP_ASSERT(nsi_appcb_get_ud(&appcb_, serviceId, instanceId) == nullptr,
                               "register find service handle error: invoke stack set callback userdata repeat",
                               GenArg(serviceId), GenArg(instanceId));
                NSOMEIP_ASSERT(nsi_appcb_set_ud(&appcb_, serviceId, instanceId, this),
                               "register find service handle error: invoke stack set callback userdata failed",
                               GenArg(serviceId), GenArg(instanceId));
                NSOMEIP_ASSERT(nsi_appcb_set_available(&appcb_, serviceId, instanceId, Impl::OnServiceAvailable),
    ...
    void UnregisterFindServiceHandle(FindServiceHandle const& findHandle) noexcept override
    {
        ...
        if (enableUnsetAvailable) {
            ...
            for (auto it{rMappers.begin()}; it != rMappers.end(); ++it) {
                ...
                auto serviceId{mapper->GetServiceId()};
                auto instanceId{*optionalId};
                NSOMEIP_CHECK(nsi_appcb_get_ud(&appcb_, serviceId, instanceId) != nullptr,
                              "unregister find service handle error: invoke stack unset callback userdata repeat",
                              GenArg(serviceId), GenArg(instanceId));
                NSOMEIP_CHECK(nsi_appcb_set_available(&appcb_, serviceId, instanceId, nullptr),
                              "unregister find service handle error: invoke stack unset callback failed",
                              GenArg(serviceId), GenArg(instanceId));
                NSOMEIP_CHECK(nsi_appcb_set_ud(&appcb_, serviceId, instanceId, nullptr),
    ...
    void RequestService(ServiceInfo::InstanceUID const& instanceUID) noexcept
    {
        ...
        if (enableRequest) {
            ...
            NSOMEIP_ASSERT(nsi_appcb_set_subscribe_ack(&appcb_, std::get< 0 >(instanceUID), std::get< 1 >(instanceUID),
                                                       Impl::OnSubscribeAck),
                           "request service error: invoke stack set subscribe ack callback failed",
                           GenArg(instanceUID));
            NSOMEIP_ASSERT(nsi_app_request_service(&app_, std::get< 0 >(instanceUID), std::get< 1 >(instanceUID),
                                                   std::get< 2 >(instanceUID), std::get< 3 >(instanceUID)),
    ...
    void ReleaseService(ServiceInfo::InstanceUID const& instanceUID) noexcept
    {
        ...
        auto enableRelease{false};
        {
            ...
            NSOMEIP_CHECK(it != requestedServices_.end(), "release service error: find requested services failed",
                          GenArg(instanceUID));
        ...
        if (enableRelease) {
            ...
            NSOMEIP_CHECK(nsi_app_release_service(&app_, std::get< 0 >(instanceUID), std::get< 1 >(instanceUID),
                                                  std::get< 2 >(instanceUID), std::get< 3 >(instanceUID)),
                          "release service error: invoke stack release service failed", GenArg(instanceUID));
            NSOMEIP_CHECK(
                nsi_appcb_set_subscribe_ack(&appcb_, std::get< 0 >(instanceUID), std::get< 1 >(instanceUID), nullptr),
...
private:
    ...
    /// @param[in] cred Credential information
    /// @return Result -- 0: success; !0: failure;
    static int32_t OnIAMFindService(nsi_app_t* a,
                                    void* ud,
                                    nsi_serv_t serv,
                                    nsi_inst_t inst,
                                    nsi_version_t major,
    ...
    /// @param[in] cred Credential information
    /// @return Result -- 0: success; !0: failure;
    static int32_t OnServiceAvailable(nsi_app_t* a,
                                      void* ud,
                                      int32_t avail,
                                      nsi_serv_t serv,
                                      nsi_inst_t inst,
    ...
    int32_t _handleServiceAvailable(ServiceInfo::InstanceUID const& instanceUID, int32_t available) noexcept
    {
        ...
        // Update the list of services that are valid (available and supported by the application)
        if (instanceAvailable) {
            std::lock_guard< Mutex > lock(availableInstanceInfosMutex_);
            auto itPair{availableInstanceInfos_.emplace(instanceUID)};
            if (!itPair.second) {
        ...
        // Get the service information supported by the current application from the registered service map by service identifier
        auto itMapper{rMappers_.find(std::get< 0 >(instanceUID))};
        NSOMEIP_ASSERT(itMapper != rMappers_.end(), GenArg(instanceUID));
        auto& mapper{itMapper->second};
        // Callback the registered discovery service handle set (associated discovery service handler)
        ...
        for (auto it{rMappers.begin()}; it != rMappers.end(); ++it) {
            ...
            // Callback discovery service handle set (specified instance identifier)
            ...
            if (serviceInfo != nullptr) {
                ...
                for (auto it2{itPair.first}; it2 != itPair.second;) {
                    ...
                    // Determine whether the currently available service matches the requirement
                    if (instanceAvailable && _verifyServiceAvailable(rServiceInfo, instanceUID)) {  // Notify that the specified instance is available
                        ...
                        ComLogTrace("handle service available: invoke callback handle specific instance available done",
                                    GenArg(instanceUID), GenArg(findServiceHandle));
                    } else {  // Notify that the specified instance is unavailable
            ...
            // Callback discovery service handle set (any instance identifier)
            ...
            // Callback discovery service handle set (any binding instance identifier)
            auto anyNSomeipInstanceIdentifier{runtime::ToInstanceIdentifier(Message::kInstanceIdAny)};
            {
                std::lock_guard< Mutex > lock(findHandle2CBDataMutex_);
                auto itPair{findHandle2CBData_.equal_range({serviceIdentifier, anyNSomeipInstanceIdentifier, 0})};
                for (auto it2{itPair.first}; it2 != itPair.second;) {
                    auto itCurrent{it2++};
                    auto findServiceHandle{itCurrent->first};
                    auto findHandleCBData{itCurrent->second};
    ...
    /// @brief Verify whether the given service instance matches the required service information
...
public:
                      ...
                      std::function< void(int32_t rcode) >&& callback) noexcept
    {
        ...
        NSOMEIP_CHECK(eid != Message::kMethodIdAny, "subscribe stack event refused: event id invalid",
                      GenArg(serviceId), GenArg(instanceId), GenArg(major), GenArg(eid));
        int32_t result{};
        {
        ...
        auto ret{NSOMEIP_CHECK(nsi_app_request_event(&app_, serviceId, instanceId, eid, ev),
        ...
        ret = NSOMEIP_CHECK(nsi_app_subscribe(&app_, serviceId, instanceId, ev->groups[0], major, eid),
                        ...
                        nsi_event_info_t* ev) noexcept
    {
        ...
        NSOMEIP_CHECK(eid != Message::kMethodIdAny, "unsubscribe stack event refused: event id invalid",
                      GenArg(serviceId), GenArg(instanceId), GenArg(major), GenArg(eid));
        int32_t result{};
        auto ret{NSOMEIP_CHECK(nsi_app_unsubscribe(&app_, serviceId, instanceId, ev->groups[0], major, eid),
                               "unsubscribe stack event error: invoke stack unsubscribe failed", GenArg(serviceId),
                               GenArg(instanceId), GenArg(major), GenArg(eid))};
        if (ret < 0) {
            result = ret;
        }
        ret = NSOMEIP_CHECK(nsi_app_release_event(&app_, serviceId, instanceId, eid),
        ...
        }
        {
            std::lock_guard< std::mutex > lock(activeGroupsMutex_);
            auto& groupInfo{activeGroups_[{serviceId, instanceId}]};
            ret = NSOMEIP_CHECK(groupInfo.ackId2Listener.erase(eid) == 1,
...
private:
#ifdef HAS_ARA_IAM
    /// @brief Receive event subscription
    /// @param[in] a Application layer application
    /// @param[in] ud User data
    ...
    /// @param[in] cred Credential information
    /// @return Result -- 0: success; !0: failure;
    static int32_t OnIAMSubscribe(nsi_app_t* a,
                                  void* ud,
                                  nsi_cid_t cid,
                                  nsi_serv_t serv,
                                  nsi_inst_t inst,
                                  ...
                                  nsi_cred_t const* cred) noexcept
    {
        ...
        switch (cred->type) {
            ...
            case NSI_CRED_ADDRESS:
                ...
                break;
            case NSI_CRED_LOCAL:  // NOLINT -- Compatibility >[bugprone-branch-clone]
                break;
            case NSI_CRED_CERT:
                break;
            default:
...
#else
#endif
    /// @brief Receive event subscription -- skeleton side
    /// @param[in] a Application layer application
    /// @param[in] ud User data
    /// @param[in] cid Client identifier
    /// @param[in] serv Service identifier
    ...
    /// @param[in] cred Credential information
    /// @return Result -- 0: success; !0: failure;
    static int32_t OnSubscribe(nsi_app_t* a,
                               void* ud,
                               nsi_cid_t cid,
                               nsi_serv_t serv,
                               nsi_inst_t inst,
    ...
    /// @brief Process event subscription -- skeleton side
    /// @param[in] cid Client identifier
    /// @param[in] serviceId Service identifier
    /// @param[in] instanceId Instance identifier
    /// @param[in] gid Event group identifier
                             ...
                             ServiceInfo::EventId eid) noexcept
    {
        ...
        MonitorTrace(runtime::GetInstance(), trace::someip::kEventSubscribed, trace::ServiceType::kSkeleton, serviceId,
                     instanceId, ara::com::internal::ServiceIdentifier("unknown"), eid, "unknown", false, 0);
        return NSOMEIP_CHECK(nsi_app_subscribe_ack(&app_, Message::kReturnCodeOk, cid, serviceId, instanceId, gid, eid),
    ...
    /// @brief Receive event subscription confirmation -- proxy side
    /// @param[in] a Application layer application
    /// @param[in] ud User data
    ...
    static int32_t OnSubscribeAck(
        nsi_app_t* a, void* ud, int32_t rcode, nsi_serv_t serv, nsi_inst_t inst, nsi_gid_t gid, nsi_eid_t eid) noexcept
    {
        std::ignore = a;
        std::ignore = ud;
    ...
    /// @brief Process event subscription confirmation -- proxy side
    /// @param[in] rcode Return code
    /// @param[in] serviceId Service identifier
    /// @param[in] instanceId Instance identifier
    /// @param[in] gid Event group identifier
                                ...
                                ServiceInfo::EventId eid) noexcept
    {
        ...
        ara::core::Vector< std::function< void(int32_t) > > callbacks;
        {
            ...
            if (eid == Message::kMethodIdAny) {
                ...
                // Notify the specified event belonging to this event group
...
public:
    ...
    /// @param[in] m Message
    /// @return Result -- 0: success; !0: failure;
    int32_t SendMessage(nsi_message_t* m) noexcept
    {
        TimeRecorder(TimeNM::Timer, "NSISendMessage");
        ComLogTrace("send message", GenArg(m));
        auto ret{NSOMEIP_CHECK(nsi_app_send(&app_, m), "send message error: invoke stack send failed", GenArg(m))};
        if (ret < 0) {
            std::ignore = nsi_message_close(m);
        }
        return ret;
    }
...
private:
    ...
    /// @param[in] m Message
    /// @return Result -- 0: success; !0: failure;
    static int32_t OnIAMMessage(nsi_app_t* a, void* ud, nsi_message_t* m) noexcept
    {
        std::ignore = a;
        std::ignore = ud;
...
NSomeipRuntime::ProxyMapperBase::ProxyMapperBase() noexcept { ComLogTrace(""); }
NSomeipRuntime::ProxyMapperBase::~ProxyMapperBase() noexcept { ComLogTrace(""); }
NSomeipRuntime::SkeletonMapperBase::SkeletonMapperBase() noexcept { ComLogTrace(""); }
NSomeipRuntime::SkeletonMapperBase::~SkeletonMapperBase() noexcept { ComLogTrace(""); }

NSomeipRuntime::ServiceRequestedHolder::ServiceRequestedHolder(ServiceInfo::InstanceUID instanceUIDIn) noexcept
    : instanceUID{std::move(instanceUIDIn)}
{
    auto& bindRuntime{runtime::GetInstance()};
    bindRuntime.impl_->RequestService(instanceUID);
}
NSomeipRuntime::ServiceRequestedHolder::~ServiceRequestedHolder() noexcept
{
    auto& bindRuntime{runtime::GetInstance()};
    bindRuntime.impl_->ReleaseService(instanceUID);
}

NSomeipRuntime::NSomeipRuntime() noexcept { ComLogTrace("create bind runtime"); }
NSomeipRuntime::~NSomeipRuntime() noexcept { ComLogTrace("destroy bind runtime"); }
ara::core::Result< void > NSomeipRuntime::Initialize() noexcept
{
    using Result = ara::core::Result< void >;
...
ara::core::Result< void > NSomeipRuntime::Deinitialize() noexcept
{
    using Result = ara::core::Result< void >;
...
void NSomeipRuntime::_registerProxyMapper(std::shared_ptr< ProxyMapperBase > const& mapper) noexcept
{
    impl_->RegisterProxyMapper(mapper);
}
void NSomeipRuntime::_registerSkeletonMapper(std::shared_ptr< SkeletonMapperBase > const& mapper) noexcept
{
    impl_->RegisterSkeletonMapper(mapper);
}
ServiceInfo* NSomeipRuntime::GetPServiceInfo(decltype(ServiceInfo::serviceId) serviceId,
                                             decltype(ServiceInfo::instanceId) instanceId) const noexcept
{
    return impl_->GetPServiceInfo(serviceId, instanceId);
}
ServiceInfo* NSomeipRuntime::GetRServiceInfo(decltype(ServiceInfo::serviceId) serviceId,
                                             decltype(ServiceInfo::instanceId) instanceId) const noexcept
{
    return impl_->GetRServiceInfo(serviceId, instanceId);
}
nsi_app_t* NSomeipRuntime::GetApp() noexcept { return impl_->GetApp(); }
nsi_appcb_t* NSomeipRuntime::GetAppCB() noexcept { return impl_->GetAppCB(); }
int32_t NSomeipRuntime::OfferService(ServiceInfo::InstanceUID const& instanceUID) noexcept
{
    return impl_->OfferService(instanceUID);
}
int32_t NSomeipRuntime::StopService(ServiceInfo::InstanceUID const& instanceUID) noexcept
{
    return impl_->StopService(instanceUID);
}
int32_t NSomeipRuntime::SubscribeEvent(ServiceInfo::ServiceId serviceId,
                                       ServiceInfo::InstanceId instanceId,
                                       ServiceInfo::MajorVersion major,
                                       ServiceInfo::EventId eid,
                                       nsi_event_info_t* ev,
...
int32_t NSomeipRuntime::UnsubscribeEvent(ServiceInfo::ServiceId serviceId,
                                         ServiceInfo::InstanceId instanceId,
                                         ServiceInfo::MajorVersion major,
                                         ServiceInfo::EventId eid,
                                         nsi_event_info_t* ev) noexcept
...
int32_t NSomeipRuntime::Send(nsi_message_t* m) noexcept { return impl_->SendMessage(m); }
}  // namespace nsomeip
}  // namespace internal
}  // namespace com
}  // namespace ara