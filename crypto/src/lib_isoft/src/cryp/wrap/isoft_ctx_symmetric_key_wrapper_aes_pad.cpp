...
/// File description:
/// ----------------
/// @file       isoft_ctx_symmetric_key_wrapper_aes_pad.cpp
...
/// @returns Expected granularity (block size)
std::size_t PCtxSymmetricKeyWrapperAesPad::GetTargetKeyGranularity() const noexcept
{
    return static_cast< std::size_t >(kInt_1U);
}
...
ara::core::Result< ara::core::Vector< ara::core::Byte > > PCtxSymmetricKeyWrapperAesPad::DoUnwrap(
    ReadOnlyMemRegion const &wrappedKey) const noexcept
{
    uint8_t const *const pData{wrappedKey.data()};
    size_t const nSize{wrappedKey.size()};
    if ((pData == nullptr) || (nSize == 0U)) {
        return ara::core::Result< ara::core::Vector< ara::core::Byte > >::FromError(
            SecurityErrorDomain::Errc::kLogicFault);
    }
    if (_GetSlotId() != 0U) {
        return _DoWrapAndUnWrapIpc(FUNC_NAME_Wrap(DoUnwrapKey_Pad), pData, static_cast< uint32_t >(nSize));
    }
...
ara::core::Result< ara::core::Vector< ara::core::Byte > > PCtxSymmetricKeyWrapperAesPad::DoWrapKeyMaterial(
    RestrictedUseObject const &key) const noexcept
{
    ...
    if (_GetSlotId() != 0U) {  /// ipc
        if (nSlotId != 0U) {
            ...
            return _DoWrapAndUnWrapIpc(FUNC_NAME_Wrap(DoWrapKeyMaterial_Pad), nullptr, 0U, nSlotId);
        }
        /// Use IPC to encrypt non-IPC
        return _DoWrapAndUnWrapIpc(FUNC_NAME_Wrap(DoWrapKeyMaterial_Pad),
    ...
    if (nSlotId != 0U) {
        ...
        return _DoWrapAndUnWrapIpc(FUNC_NAME_Wrap(DoWrapKeyMaterial_Pad), pKeyData, static_cast< uint32_t >(nKeySize),
                                   nSlotId);
    }
...
ExtensionService::Uptr PCtxSymmetricKeyWrapperAesPad::GetExtensionService() const noexcept
{
    return {std::make_unique< PServiceExtension< PCtxSymmetricKeyWrapperAesPad > >(*this)};
}

/// @brief Return a CryptoPrimitivId instance containing the instance identifier.
/// @returns CryptoPrimitiveId instance
CryptoPrimitiveId::Uptr PCtxSymmetricKeyWrapperAesPad::GetCryptoPrimitiveId() const noexcept
{
    return std::make_unique< PAlgId_WrapAesPad >();
}
...
/// @return Wrapping result
ara::core::Result< ara::core::Vector< ara::core::Byte > > PCtxSymmetricKeyWrapperAesPad::DoWrap(
    uint8_t const *pInputData, uint32_t nDataLen) const noexcept
{
    ...
    std::size_t const len{AES_wrap_key_pad(pKeyData, nullptr, static_cast< uint8_t * >(plaintext),
    ...
    uint8_t const *pInputData, uint32_t nDataLen) const noexcept
{
    ...
    uint8_t const *const pKeyData{pAesKey->GetKeySymmetric()};
    size_t const nkeyLen{pAesKey->GetPayloadSize()};
    std::size_t const len{AES_unwrap_key_pad(pKeyData, nullptr,
