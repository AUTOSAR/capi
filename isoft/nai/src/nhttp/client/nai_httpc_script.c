...
static nai_sult_t nai_httpc_prepare_values(nai_httpc_request_t* r)
{
    nai_int_t n;
    nai_int_t count;
    nai_sult_t rc;
...
nai_sult_t nai_httpc_script_exec(
    nai_httpc_request_t* r, const void* code, nai_script_value_t* sp)
{
    nai_sult_t rc;
    nai_httpc_t* h;
    nai_script_t s;
...
nai_sult_t nai_httpc_expn_value(
    nai_httpc_request_t* r, const nai_script_expn_t* e, 
...
nai_sult_t nai_httpc_command_expn(
    nai_command_t* c, nai_command_args_t* args)
{
    ...
    rc = nai_httpc_expn_compile(h, e, &str);
    if (rc < 0) {
        nai_log_error(NAI_LOG_CORE, 
            nai_sult_to_errno(rc), 
...
static nai_sult_t nai_httpc_var_not_found(
...
nai_sult_t nai_httpc_regex_compile(
    nai_httpc_t* h, nai_httpc_regex_t* re, 
    const nai_str_t* pattern, nai_int_t flags, nai_regex_errinfo_t* ei)
{
    ...
    for (n = 0; n < rn.count; n ++) {
        ...
        rc = nai_script_add_variable(
            &h->vars, &name, nai_httpc_var_not_found, 0, 0, 0);
        if (rc < 0) {
            goto _end;
        };
...
nai_sult_t nai_httpc_regex_exec(
    nai_httpc_request_t* r, nai_httpc_regex_t* re, const nai_mem_t* str)
{
    nai_int_t n;
    nai_int_t c;
    nai_int_t len;
