...
static nai_sult_t nai_https_local_init(
    nai_https_t* h, nai_local_t* l, size_t offset, nai_pool_t* p)
{
    nai_int_t n;
    void* data;
    nai_sult_t rc;
    ...
    nai_https_local_alloc_f alloc;
    nai_https_local_ctx_t ctx;
    nai_https_conf_t cf = {0};
    ...
    for (n = 0; n < (intptr_t)nai_module_map_count(&m->modules); n ++) {
        ...
        alloc = *(nai_https_local_alloc_f*)((uint8_t*)c + offset);
        if (alloc == 0) {
            nai_local_at(l, *e) = 0;
            continue;
        };
        ...
        if (rc < 0) {
            nai_log_alert(NAI_LOG_HTTPS, 
                ...
                "https modules '%s' alloc local data failed", e->name);
            goto _end;
        };
...
static nai_sult_t nai_https_local_merge(
    nai_https_t* h, nai_https_conf_t* cf, nai_local_t* l, 
    ...
    nai_pool_t* p, nai_pool_t* pt)
{
    ...
    for (n = 0; n < (intptr_t)nai_module_map_count(&m->modules); n ++) {
        ...
        merge = *(nai_https_local_merge_f*)((uint8_t*)c + offset);
        if (merge == 0) {
            continue;
        };
        ...
        if (u == 0) {
            ...
            if (data_u == 0) {
                ...
                nai_log_alert(NAI_LOG_HTTPS, 
                    nai_sult_to_errno(rc), 
                    "get local of https modules '%s' failed", e->name);
                goto _end;
            };
        };
        ...
        if (rc < 0) {
            nai_log_alert(NAI_LOG_HTTPS, 
                nai_sult_to_errno(rc), 
                "https modules '%s' combine local data failed", e->name);
            goto _end;
        };
    };
...
nai_sult_t nai_https_local_alloc(
    nai_https_t* h, nai_pool_t* p, nai_local_t** pv)
{
    nai_sult_t rc;
    nai_local_t* local;
...
nai_sult_t nai_https_service_create(
    nai_main_t* m, nai_pool_t* p, nai_https_t** pv)
{
    nai_int_t n;
    nai_int_t modules;
    nai_sult_t rc;
    ...
    modules = nai_module_map_count_grp(
        &m->modules, NAI_MODULE_HTTPS);

    h = nai_palloc(p, sizeof(*h) + sizeof(void*)*modules);
    if (h == 0) {
        rc = nai_sult_from_errno();
        nai_log_alert(NAI_LOG_HTTPS, 
            nai_sult_to_errno(rc), "alloc nai_https_t failed");
        goto _end;
    };

    h->main = m;
    rc = nai_https_local_init(h, &h->local, 
        nai_offsetof(nai_https_module_t, module_alloc), p);
    if (rc < 0) {
        nai_log_alert(NAI_LOG_HTTPS, 
            nai_sult_to_errno(rc), "init local of nai_https_t failed");
        goto _end;
    };

    nai_local_at(&h->local, nai_https_core_module) = h;
    nai_list_init(&h->clist);
    nai_list_init(&h->cfree);
    nai_array_init(&h->servers, sizeof(nai_https_server_t*), p);
    nai_array_init(&h->listening, sizeof(nai_https_listening_t*), p);
    ...
    h->prepared = 0;
    h->min_connections = NAI_OPT_UNSET_VALUE;
    h->min_free_connections = NAI_OPT_UNSET_VALUE;
    h->nlist = 0;
    h->nfree = 0;
    ...
    for (n = 0; n < (nai_int_t)nai_countof(h->groups); n ++) {
        ...
        nai_array_init(&g->handles, 
            sizeof(nai_https_stage_entry_t), p);
    };

    ps = (nai_https_server_t**)nai_array_push(&h->servers);
    if (ps == 0) {
        rc = nai_sult_from_errno();
        nai_log_alert(NAI_LOG_HTTPS, 
    ...
    rc = nai_https_server_create(h, p, ps);
    if (rc < 0) {
        goto _end;
    };
...
nai_sult_t nai_https_server_create(
    nai_https_t* h, nai_pool_t* p, nai_https_server_t** pv)
{
    nai_sult_t rc;
    ...
    if (s == 0) {
        rc = nai_sult_from_errno();
        nai_log_alert(NAI_LOG_HTTPS, 
            nai_sult_to_errno(rc), "alloc nai_https_server_t failed");
        goto _end;
    };

    rc = nai_https_local_init(h, &s->local, 
        nai_offsetof(nai_https_module_t, server_alloc), p);
    if (rc < 0) {
        nai_log_alert(NAI_LOG_HTTPS, 
            nai_errno_to_sult(rc), "init local of nai_https_server_t failed");
        goto _end;
    };

    nai_local_at(&s->local, nai_https_core_module) = s;
    nai_str_setn(&s->name);
    nai_array_init(&s->names, sizeof(nai_https_server_name_t), p);
    nai_array_init(&s->table.list, sizeof(nai_https_location_t*), p);
    nai_array_init(&s->table.patterns, 
        sizeof(nai_https_location_entry_t*), p);
    ...
    s->read_head_timeo = NAI_OPT_UNSET_VALUE;
    s->conf.main = &h->local;
    ...
    rc = nai_https_location_create(h, p, &s->conf, pl);
    if (rc == 0) {
        nai_log_alert(NAI_LOG_HTTPS, 
...
_end:
 ...
 nai_sult_t nai_https_location_create(
    nai_https_t* h, nai_pool_t* p, nai_https_conf_t* c, 
    nai_https_location_t** pv)
{
    nai_sult_t rc;
    ...
    if (l == 0) {
        rc = nai_sult_from_errno();
        nai_log_alert(NAI_LOG_HTTPS, 
            nai_sult_to_errno(rc), "alloc nai_https_location_t failed");
        goto _end;
    };

    /* init location's local */
    rc = nai_https_local_init(h, &l->local, 
        nai_offsetof(nai_https_module_t, location_alloc), p);
    if (rc < 0) {
        rc = nai_sult_from_errno();
        nai_log_alert(NAI_LOG_HTTPS, 
            nai_sult_to_errno(rc), 
            "init local of nai_https_location_t failed");
        goto _end;
    };

    /* init location */
    nai_local_at(&l->local, nai_https_core_module) = l;
    nai_str_setn(&l->name);
    nai_str_setn(&l->default_type);
    nai_array_init(&l->types, sizeof(nai_hash_value_t), p);
    nai_hashsnap_init(&l->types_hash, p);
    nai_script_expn_init(&l->root);
    l->alias = 0;
    l->type = NAI_HTTPS_LOC_NONAME;
...
static nai_sult_t nai_https_location_tree_merge(nai_rbtree_t* t)
{
    nai_sult_t rc;
    nai_rbnode_str_t* e;
    nai_rbnode_str_t* end;
    nai_https_location_t* l;
    nai_https_location_node_t* n;


    e = nai_rbtree_str_begin(t);
    end = nai_rbtree_str_end(t);
    while (e != end) {
        n = (nai_https_location_node_t*)e;
        if (n->exact) {
            l = n->exact;
            rc = nai_https_location_merge(l, l->parent, 0, 0);
            if (rc < 0) {
                nai_log_alert(NAI_LOG_HTTPS, 
                    nai_sult_to_errno(rc), 
        ...
        rc = nai_https_location_tree_merge(&n->sub);
        if (rc < 0) {
            goto _end;
        };
    };
...
static nai_sult_t nai_https_location_tree_build(nai_rbtree_t* t)
{
    nai_rbnode_str_t* e;
    nai_rbnode_str_t* end;
    nai_https_location_node_t* n;
    nai_https_location_node_t* c;
...
static nai_sult_t nai_https_locations_build(
    nai_https_t* h, nai_https_locations_t* t)
{
    nai_int_t i;
    nai_sult_t rc;
    nai_main_t* m = h->main;
    nai_pool_t* p = &m->pool;
    nai_pool_t* pt = nai_main_get_temp_pool(m);
#if (NAI_HAVE_REGEX)
    ...
    nai_https_location_t** pl;
    nai_https_location_node_t* n;
    nai_https_location_entry_t* e;


    /* scan all locations */
    pl = (nai_https_location_t**)t->list.elts;
    for (i = 1; i < (intptr_t)t->list.count; i ++) {
        l = pl[i];
        switch (l->type) {
        case NAI_HTTPS_LOC_EXACT:
        case NAI_HTTPS_LOC_NORMAL:
        case NAI_HTTPS_LOC_PRIORITY:
            n = (nai_https_location_node_t*)
                nai_rbtree_str_find(&t->named, &l->name, 0);
            if (n == 0) {
                n = (nai_https_location_node_t*)nai_palloc(p, sizeof(*n));
                if (n == 0) {
                    rc = nai_sult_from_errno();
                    nai_log_alert(NAI_LOG_HTTPS, 
        ...
        rc = nai_https_location_merge(l, l->parent, p, pt);
        if (rc < 0) {
            nai_log_alert(NAI_LOG_HTTPS, 
                nai_sult_to_errno(rc), 
        ...
        case NAI_HTTPS_LOC_PATTERN_CASE:
            e = (nai_https_location_entry_t*)nai_palloc(p, sizeof(*e));
            if (e == 0) {
                rc = nai_sult_from_errno();
                nai_log_alert(NAI_LOG_HTTPS, 
                    nai_sult_to_errno(rc), "alloc location entry failed");
            ...
            e->loc = l;
            rc = nai_regex_compile(&e->pattern, nai_str(&l->name), 
                l->type == NAI_HTTPS_LOC_PATTERN ? NAI_REGEX_CASELESS:0 | 
                NAI_REGEX_JIT, &err, p);
            if (rc < 0) {
                rc = nai_sult_from_errno();
    ...
    rc = nai_https_location_tree_build(&t->named);
    if (rc < 0) {
        goto _end;
    };

    rc = nai_https_location_tree_merge(&t->named);
    if (rc < 0) {
        goto _end;
    };
...
nai_sult_t nai_https_server_merge(
    nai_https_server_t* s, nai_https_server_t* up, 
    nai_pool_t* p, nai_pool_t* pt)
{
    nai_sult_t rc;
    nai_https_t* h;
    ...
    if (up == 0) {
        rc = nai_https_local_merge(h, 
            &s->conf, s->conf.server, 0, 
            nai_offsetof(nai_https_module_t, server_merge), 1, p, pt);
        if (rc < 0) {
            nai_log_error(NAI_LOG_HTTPS, 
                nai_sult_to_errno(rc), 
        ...
        rc = nai_https_local_merge(h, 
            ...
            nai_offsetof(nai_https_module_t, location_merge), 1, p, pt);
        if (rc < 0) {
            nai_log_error(NAI_LOG_HTTPS, 
                nai_sult_to_errno(rc), 
    ...
    rc = nai_https_locations_build(h, &s->table);
    if (rc < 0) {
        nai_log_error(NAI_LOG_HTTPS, 
            nai_sult_to_errno(rc), 
...
nai_sult_t nai_https_location_merge(
    nai_https_location_t* s, nai_https_location_t* up, 
    nai_pool_t* p, nai_pool_t* pt)
{
    nai_sult_t rc;
    nai_https_t* h;
    ...
    if (s->type != NAI_HTTPS_LOC_NONAME) {
        sibling = s->sibling;
        while (sibling) {
            rc = nai_https_local_merge(h, 
