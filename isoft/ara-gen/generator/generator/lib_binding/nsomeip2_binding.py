...
class Nsomeip2Binding:
    def __init__(self, renderer, output_dir, machine_dir,processes_dir) -> None:
        ...
        self.nsomeip2_root_dir = output_dir#os.path.join(output_dir, 'nsomeip')
        self.machine_config_root_dir = machine_dir
        self.processes_root_dir = processes_dir

    def nsomeip2_service_dir(self, service: ServiceView):
        if service.lowNamespaces is not None:
            repath = os.path.join(self.nsomeip2_root_dir ,*service.lowNamespaces)
            return os.path.join(repath,"nsomeip")
        else:
            return self.nsomeip2_root_dir

    def write_guarded_file(self, template, service_view, filename, **kwargs):
        output_dir = self.nsomeip2_service_dir(service_view)
        self._renderer.write_output(
            template,
            output_dir,
            filename,
            header_base=self._renderer.include_guard(service_view.namespaces+["nsomeip"], filename),
            **kwargs
        )
    ...
    @handle_method_exceptions(error_string_template="NSOMEIP2 service description generation ERROR: {exception_text}")
    def generate_service_desc(self, service_interface):
        """Generate the cpp and h files for service description"""
        si_desc_template_h = self.load_template("nsomeip2_binding/service_desc_xxx_h.j2")
        for stoemapping in service_interface.stoemappings:
            for etosmapping in service_interface.etosmappings:
                if etosmapping.event == stoemapping.event[0] and etosmapping.signals[0] == stoemapping.signal and etosmapping.eventtype == stoemapping.eventtype:
                    etosmapping.set_EinS(True)
    ...
    @handle_method_exceptions(error_string_template="NSOMEIP2 JSON config generation ERROR: {exception_text}")
    def generate_nsomeip2_json(self, machine, services):
        """Generate the nsomeip2 json file required by genivi nsomeip2"""
        nsomeip2_template = self.load_template("nsomeip2_binding/app_nsomeip2_json.j2")

        output_filename = "{}_nsomeip.json".format(machine.name)
        self._renderer.write_output(
            nsomeip2_template,
    ...
    def generate_proxy(self, service: ServiceView):
        ...
        proxy_nsomeip2_template = self.load_template("nsomeip2_binding/proxy_nsomeip2_xxx_h.j2")
        ...
        self.write_guarded_file(proxy_nsomeip2_template, service, output_filename, service=service)

    def generate_si_adapter(self, service: ServiceView):
        """Generate the adapter_<name>.h file for the service interface"""
        si_adapter_template = self.load_template("nsomeip2_binding/skeleton_impl_h.j2")
    ...
    def generate_machine_config(self, machine, provided_services):
        self.generate_nsomeip2_json(machine, provided_services)
