...
class SERNotificalInterface(object):
    """Describes a SERNotificalInterface that can be provided by
    or requested from a component"""
    def __init__(self, fqn: str) -> None:
        if fqn == "":
            raise AttributeError("A SERNotificalInterface must have a name")
    ...
    def __str__(self):
        return "SERNotificalInterface(%s)" % (self.name)
    ...
    @property
    def name(self) -> str:
        """Get the SERNotificalInterface name"""
