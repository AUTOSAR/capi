...
static int npc_port_seen(npc_port_t* p)
{
    ...
    diff = (u[p->in.slot].next + t->qcount - i) % (t->qcount);
    batch = (t->qcount > 10) ? 10 : t->qcount - 1;
...
static npc_message_t* npc_port_unpack_message(npc_port_t* p, npc_mref_t msg)
{
    ...
    if (pak->hdr.method == NSI_MT_NOTIFICATION && cif->ops) {
    ...
    m = nsi_network_create_message(cif->com->ud, NPC_USAGE_READ);
    if (m == 0) {
