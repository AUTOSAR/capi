...
#ifndef _IPC_BASE_C_H_
#define _IPC_BASE_C_H_
...
extern "C"
{
    ...
    ////
    nai_fd_t ipc_base_socket_get(void);

    /// @brief Create a socket for IPC communication.
    /// @return NAI_FD_INVALID, failure, and set nai_errno; nai_fd_t, success.
    /// @note The socket returned by this function is a datagram-oriented UNIX domain socket and has been bound.
    ////
    nai_fd_t ipc_base_socket_create(const char *socketName);
    ...
    ///        The current implementation of this function is based on a static algorithm, so it cannot verify the correctness of processName. Therefore, reasons for call failure are: 1. Invalid processName address; 2. Invalid buffer address; 3. Buffer too small.
    ////
    int ipc_base_socket_resolve_name(const char *processName, char *ipcName, unsigned int nameLen);
